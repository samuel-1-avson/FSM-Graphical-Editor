...
class ChatbotWorker(QObject):
    ...
    def set_diagram_context_slot(self, diagram_json_str: str):
        if not diagram_json_str:
            ...
            # Get the current active editor to add its context.
            editor = self.parent_window.current_editor() if hasattr(self.parent_window, 'current_editor') else None
            diagram_json_str_ctx = ""
            if editor:
                try:
                    diagram_data = editor.scene.get_diagram_data()
                    diagram_json_str_ctx = json.dumps(diagram_data)
                except Exception as e:
                    logger.error(f"Error getting diagram context in worker: {e}", exc_info=True)
            self.current_diagram_context_json_str = diagram_json_str_ctx
...
class AIChatUIManager(QObject):
    ...
    # --- REPLACED: Use markdown-it for robust parsing ---
    def _render_tokens_to_styled_html(self, tokens: list[SyntaxTreeNode]) -> str:
        ...
        """
        # Define basic styling for different markdown elements
        ...
        def render_code_inline(self, tokens: list, idx: int, options: dict, env: dict) -> str:
            return f'<code style="{inline_code_style}">{html.escape(tokens[idx].content)}</code>'

        def render_blockquote_open(self, tokens: list, idx: int, options: dict, env: dict) -> str:
            return f'<blockquote style="{blockquote_style}">'

        # You can add more overrides here, for example, to style headings:
        # def render_heading_open(self, tokens: list, idx: int, options: dict, env: dict) -> str:
        #     tag = tokens[idx].tag # e.g., 'h1', 'h2'
        #     return f'<{tag} style="color: {COLOR_ACCENT_PRIMARY}; margin-bottom: 2px;">'
        ...
        # Restore original renderers to avoid side effects if the parser is reused elsewhere
        if original_code_inline_renderer:
            self.md_parser.renderer.rules["code_inline"] = original_code_inline_renderer
        if original_blockquote_open_renderer:
             self.md_parser.renderer.rules["blockquote_open"] = original_blockquote_open_renderer
    ...
    def _append_to_chat_display(self, sender: str, message: str):
        ...
        for match in code_block_regex.finditer(message):
            # Parse and render the text part before the code block
            text_before = message[last_index:match.start()]
            if text_before.strip():
                tokens = self.md_parser.parse(text_before.strip())
                processed_message_html_parts.append(self._render_tokens_to_styled_html(tokens))

            # Format the code block itself
            language = match.group(1).strip()
            code_content = match.group(2).strip('\n')
            processed_message_html_parts.append(self._format_code_block(code_content, language))
            last_index = match.end()

        # Parse and render any remaining text after the last code block
        text_after = message[last_index:]
        if text_after.strip():
            tokens = self.md_parser.parse(text_after.strip())
            processed_message_html_parts.append(self._render_tokens_to_styled_html(tokens))
...
class AIChatbotManager(QObject):
    ...
    def _setup_worker(self):
        ...
        self.chatbot_worker = ChatbotWorker(self.api_key, parent=self.parent_window) # Pass parent to worker
