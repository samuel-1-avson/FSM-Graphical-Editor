...
class StateMachinePoweredSimulator:
        ...
        def dynamic_callback_wrapper(*args, **kwargs_from_sm_call):
            ...
            try:
                if callback_type == "action":
                    ...
                    # After executing the code, we must update the simulator's master
                    # variable dictionary with any new or changed variables from the
                    # temporary execution scope.
    ...
    def reset(self):
        ...
        if self.FSMClass:
            try:
                self.sm = self.FSMClass(model=self._variables) 
                current_state_id = self.sm.current_state.id if self.sm and self.sm.current_state else 'None'
    ...
    def step(self, event_name=None):
        if self.simulation_halted_flag:
            ...
            return self.get_current_leaf_state_name(), self.get_last_executed_actions_log()
        if self.paused_on_breakpoint:
            self._log_action(f"Simulation PAUSED at breakpoint. Current state: {self.get_current_leaf_state_name()}. Event '{event_name or 'None'}' ignored. Use 'Continue'.")
            return self.get_current_leaf_state_name(), self.get_last_executed_actions_log()
        if not self.sm: 
            if self.parent_simulator : self._log_action("Step ignored: Sub-FSM is empty/not initialized.")
            else: self._log_action("Step ignored: FSM not initialized."); self.simulation_halted_flag = True;
            return self.get_current_leaf_state_name(), self.get_last_executed_actions_log()

        self.current_tick += 1
        main_state_id = self.sm.current_state.id if self.sm and self.sm.current_state else "Uninitialized"
        self._log_action(f"--- Step. State: {self.get_current_leaf_state_name()}. Event: {event_name or 'Internal (Tick)'} ---")
        
        if self.breakpoint_hit_flag:
            self.paused_on_breakpoint = True
            self.breakpoint_hit_flag = False
            self._log_action(f"Simulation PAUSED at breakpoint. Current state: {self.get_current_leaf_state_name()}")
            return self.get_current_leaf_state_name(), self.get_last_executed_actions_log()

        try:
            if self.breakpoint_hit_flag: 
                self.paused_on_breakpoint = True; self.breakpoint_hit_flag = False
                self._log_action(f"Simulation PAUSED at breakpoint (hit during pre-step hook). Current state: {self.get_current_leaf_state_name()}"); 
                return self.get_current_leaf_state_name(), self.get_last_executed_actions_log()
            ...
            if self.active_sub_simulator:
                ...
                if self.active_sub_simulator.simulation_halted_flag: self.simulation_halted_flag = True; self._log_action(f"Propagation: Parent HALTED due to sub-machine error in '{superstate_log_name}'."); return self.get_current_leaf_state_name(), self.get_last_executed_actions_log()
                if self.active_sub_simulator.paused_on_breakpoint: self.paused_on_breakpoint = True; self._log_action(f"Propagation: Parent PAUSED because sub-machine in '{superstate_log_name}' hit a breakpoint."); return self.get_current_leaf_state_name(), self.get_last_executed_actions_log()
            ...
            if self.breakpoint_hit_flag: 
                self.paused_on_breakpoint = True; self.breakpoint_hit_flag = False
                self._log_action(f"Simulation PAUSED at breakpoint (hit during sub-machine step). Current state: {self.get_current_leaf_state_name()}");
                return self.get_current_leaf_state_name(), self.get_last_executed_actions_log()
            ...
            if self.breakpoint_hit_flag:
                self.paused_on_breakpoint = True
                self.breakpoint_hit_flag = False
                self._log_action(f"Simulation PAUSED at breakpoint (hit during/after event processing). Current state: {self.get_current_leaf_state_name()}")
        ...
        except TransitionNotAllowed: self._log_action(f"Event '{event_name}' not allowed or no transition from '{self.get_current_leaf_state_name()}'.")
