...
class MainWindow(QMainWindow):
    ...
    def _on_close_tab_requested(self, index: int):
        """Handle the user clicking the 'x' on a tab."""
        editor_to_close = self.tab_widget.widget(index)
        if not editor_to_close:
            return

        # Use a refactored prompt method
        ...
        # If no tabs are left, create a fresh untitled one
        if self.tab_widget.count() == 0:
            self.action_handler.on_new_file()

    @pyqtSlot(int)
    def _on_current_tab_changed(self, index: int):
        """Update global state when the user switches tabs."""
        editor = self.current_editor()
        is_editor_active = editor is not None
        ...
        if is_editor_active:
            # Connect menu actions to the new active undo stack
