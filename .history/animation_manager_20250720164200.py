...
class AnimationManager(QObject):
    ...
    def __init__(self, graphics_scene, main_window=None):
        ...
        self._highlighted_state: GraphicsStateItem | None = None
    ...
    def animate_state_exit(self, state_name: str):
        ...
        if state_item:
             state_item.set_py_sim_active_style(False)
        if self._highlighted_state == state_item:
    ...
    def animate_transition(self, from_state: str, to_state: str, event: str):
        ...
        for item in editor.scene.items():
            if isinstance(item, GraphicsTransitionItem):
                if (item.start_item and item.start_item.text_label == from_state and
                    item.end_item and item.end_item.text_label == to_state and
                    item.event_str == event):
                    transition_item = item
                    break
        
        if transition_item:
            transition_item.start_pulse_animation()
    ...
    def clear_animations(self):
        if self._highlighted_state:
            self._highlighted_state.set_py_sim_active_style(False)
            self._highlighted_state = None
        
        editor = self.mw.current_editor()
        if editor:
            for item in editor.scene.items():
                if isinstance(item, GraphicsTransitionItem) and item.is_py_sim_active:
                    item.set_py_sim_active_style(False)
