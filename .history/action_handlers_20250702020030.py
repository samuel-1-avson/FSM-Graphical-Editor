...
class ActionHandler(QObject):
    def __init__(self, main_window):
        ...
        # --- NEW: Store pending request IDs and their callbacks ---
        self._pending_git_requests = {}
    ...
    def connect_actions(self):
        ...
        if hasattr(self.mw, 'git_commit_action'):
            # Connect the git manager's public signal to a handler in this class
            if self.mw.git_manager:
                self.mw.git_manager.command_result.connect(self._on_git_command_result)
    ...
    # --- NEW: Central handler for all Git command results ---
    @pyqtSlot(str, bool, str, str)
    def _on_git_command_result(self, request_id, success, stdout, stderr):
        if request_id in self._pending_git_requests:
            callback, operation_name = self._pending_git_requests.pop(request_id)
            logger.info(f"Git result received for '{operation_name}' (ID: {request_id})")
            callback(success, stdout, stderr)
        else:
            logger.warning(f"Received git result for an unknown or expired request ID: {request_id}")

    # --- GIT ACTION HANDLERS (MODIFIED) ---
    ...
    def on_git_commit(self):
        ...
        if ok and commit_message.strip():
            ...
            request_id = self.mw.git_manager.run_command_in_repo(command, file_path)
            self._pending_git_requests[request_id] = (self._on_git_add_finished_for_commit, "Git Add")
    ...
    def _on_git_add_finished_for_commit(self, success, stdout, stderr):
        ...
        # This now calls a generic handler, so we wrap the final UI update in a lambda
        request_id = self.mw.git_manager.run_command_in_repo(command, file_path)
        self._pending_git_requests[request_id] = (self._on_git_op_finished, "Git Commit")
    
    # NEW generic handler for simple Git operations
    def _on_git_op_finished(self, success, stdout, stderr):
        self.mw.main_op_status_label.setText("Ready.")
        if success:
            QMessageBox.information(self.mw, "Git Success", f"Operation completed:\n\n{stdout}")
        else:
            QMessageBox.critical(self.mw, "Git Error", f"Operation failed:\n{stderr}")
        
        # Re-check status after any operation
        file_path = self._get_current_file_path_for_git()
        if file_path:
            self.mw.git_manager.check_file_status(file_path)

    @pyqtSlot()
    def on_git_pull(self):
        file_path = self._get_current_file_path_for_git()
        if file_path:
            self.mw.main_op_status_label.setText("Pulling from remote...")
            request_id = self.mw.git_manager.run_command_in_repo(['git', 'pull'], file_path)
            self._pending_git_requests[request_id] = (self._on_git_op_finished, "Git Pull")

    @pyqtSlot()
    def on_git_push(self):
        file_path = self._get_current_file_path_for_git()
        if file_path:
            self.mw.main_op_status_label.setText("Pushing to remote...")
            request_id = self.mw.git_manager.run_command_in_repo(['git', 'push'], file_path)
            self._pending_git_requests[request_id] = (self._on_git_op_finished, "Git Push")
    ...
    def on_git_show_changes(self):
        ...
        request_id = self.mw.git_manager.run_command_in_repo(command, file_path)
        self._pending_git_requests[request_id] = (self._on_git_show_changes_finished, "Git Diff")
