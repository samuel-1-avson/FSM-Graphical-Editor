...
# --- START: NEW GENERATOR FUNCTION for Testbench ---
...
def generate_c_testbench_content(diagram_data: dict, fsm_name_c: str) -> str:
    ...
    Args:
        diagram_data: The dictionary containing the FSM structure.
        fsm_name_c: The sanitized C identifier for the FSM, used for #include.

    Returns:
        The generated C code for the testbench as a string.
    """
    ...
    # 2. Prepare Data Context for the Testbench Template
    # The context needed for the testbench is simpler than for the FSM itself.
    context = {
        "fsm_name_c": fsm_name_c,
        "app_name": "BSM Designer", # Assuming APP_NAME is not directly available here
        "app_version": "2.0+", # Assuming APP_VERSION is not directly available here
    ...
    # We need to find the initial state enum and a list of all events.
    ...
    for state_data in diagram_data.get('states', []):
        ...
        context["states"].append(state_ctx) # Pass all states for the get_state_name helper
    ...
    # If no initial state was explicitly marked, default to the first one for the assert.
    if not initial_state_found and context['states']:
        first_state_c_name = sanitize_c_identifier(context['states'][0]['name'])
        context["initial_state_c_enum"] = f"STATE_{first_state_c_name.upper()}"

    # Get a unique, sorted list of all named events.
    ...
    # 3. Render the testbench template
