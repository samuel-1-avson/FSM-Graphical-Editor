...
from PyQt6.QtWidgets import (
    QMainWindow, QDockWidget, QToolBar, QVBoxLayout, QHBoxLayout, QWidget, QLabel,
...
class UIManager(QObject):
    """
    Manages the dynamic state and interactions of the user interface.
    This class is the central authority for updating UI elements based on application state.
    """
    ...
    # --- UI STATE MANAGEMENT (MOVED FROM MAINWINDOW) ---

    def update_all_ui_element_states(self):
        """Central function to refresh the entire UI based on the current context."""
        self.update_window_title()
        self.update_undo_redo_actions()
        self.update_save_actions()
        self.update_properties_dock()
        self.mw.py_sim_ui_manager._update_internal_controls_enabled_state()
        self.update_zoom_to_selection_action()
        self.update_align_distribute_actions()
        self.mw.update_resource_estimation()
        self.mw._update_project_actions_state()
        
        editor = self.mw.current_editor()
        if editor and editor.view:
            self.mw.update_zoom_status_display(editor.view.transform().m11())
        
        self.mw._request_live_preview_update()

    def update_window_title(self):
        """Updates the main window title based on project, file, and simulation state."""
        project_name = self.mw.project_manager.project_data.get("name") if self.mw.project_manager.is_project_open() else None
        editor = self.mw.current_editor()
        title_parts = []
        ...
        if not editor:
            if not project_name:
                self.mw.setWindowTitle(config.APP_NAME)
                return
        else:
            dirty_char = "[*]" # Qt handles this automatically via setWindowModified
            file_name = os.path.basename(editor.file_path) if editor.file_path else "Untitled"
            title_parts.append(file_name)
            self.mw.setWindowModified(editor.is_dirty())
            title_parts.append(dirty_char)
    
        # Check if any tab has an active Python simulation
        py_sim_active = any(
            isinstance(self.mw.tab_widget.widget(i), self.mw.EditorWidget) and self.mw.tab_widget.widget(i).py_sim_active
            for i in range(self.mw.tab_widget.count())
        )
        pysim_suffix = " [PySim Active]" if py_sim_active else ""
    
        title_parts.append(f"- {config.APP_NAME}{pysim_suffix}")
        self.mw.setWindowTitle(" ".join(title_parts))

    def update_save_actions(self):
        """Enables or disables save actions based on the current editor's dirty state."""
        if hasattr(self.mw, 'save_action'):
            can_save = self.mw.current_editor() is not None and self.mw.current_editor().is_dirty()
            self.mw.save_action.setEnabled(can_save)
        if hasattr(self.mw, 'save_as_action'):
            self.mw.save_as_action.setEnabled(self.mw.current_editor() is not None)

    def update_undo_redo_actions(self):
        """Updates the text and enabled state of undo/redo actions."""
        editor = self.mw.current_editor()
        
        can_undo = editor is not None and editor.undo_stack.canUndo()
        can_redo = editor is not None and editor.undo_stack.canRedo()

        if not hasattr(self.mw, 'undo_action'): return

        self.mw.undo_action.setEnabled(can_undo)
        self.mw.redo_action.setEnabled(can_redo)
        
        undo_text = editor.undo_stack.undoText() if can_undo else ""
        redo_text = editor.undo_stack.redoText() if can_redo else ""
        
        self.mw.undo_action.setText(f"&Undo{(' ' + undo_text) if undo_text else ''}")
        self.mw.redo_action.setText(f"&Redo{(' ' + redo_text) if redo_text else ''}")
        self.mw.undo_action.setToolTip(f"Undo: {undo_text}" if undo_text else "Undo")
        self.mw.redo_action.setToolTip(f"Redo: {redo_text}" if redo_text else "Redo")

    def update_zoom_to_selection_action(self):
        """Enables zoom-to-selection action if items are selected."""
        editor = self.mw.current_editor()
        can_zoom = editor is not None and bool(editor.scene.selectedItems())
        if hasattr(self.mw, 'zoom_to_selection_action'):
            self.mw.zoom_to_selection_action.setEnabled(can_zoom)

    def update_align_distribute_actions(self):
        """Enables alignment/distribution actions based on selection count."""
        editor = self.mw.current_editor()
        count = len(editor.scene.selectedItems()) if editor else 0
        if hasattr(self.mw, 'align_actions'):
            for action in self.mw.align_actions:
                action.setEnabled(count >= 2)
        if hasattr(self.mw, 'distribute_actions'):
            for action in self.mw.distribute_actions:
                action.setEnabled(count >= 3)
                
    # --- DOCK POPULATION AND MANAGEMENT ---
    ...
    # ... (the rest of the UIManager's methods, such as _clear_properties_layout,
    # _display_no_selection_view, _get_property_schema_for_item, etc.,
    # remain here as they were in the original file)
