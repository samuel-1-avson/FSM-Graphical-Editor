...
from ..utils.config import (
    ...
    COLOR_BORDER_MEDIUM, MIME_TYPE_BSM_ITEMS, MIME_TYPE_BSM_TEMPLATE
)
...
class UIManager(QObject):
    ...
    def _populate_data_dictionary_dock(self):
        ...
        # --- FIX: Connect to the global signal bus here ---
    ...
    def _load_and_display_templates(self):
        ...
        for name, data in FSM_TEMPLATES_BUILTIN.items():
            btn = DraggableToolButton(data.get('name', name), MIME_TYPE_BSM_TEMPLATE, json.dumps(data))
    ...
    def _populate_properties_dock(self):
        ...
        container_layout.addWidget(mw.properties_placeholder_label)
        ...
        mw.properties_editor_layout.setContentsMargins(6, 6, 6, 6)
        mw.properties_editor_layout.setSpacing(6)
        mw.properties_editor_layout.addStretch()
        container_layout.addWidget(mw.properties_editor_container)

        mw.properties_multi_select_container = QFrame()
        mw.properties_multi_layout = QHBoxLayout(mw.properties_multi_select_container)
        mw.properties_multi_layout.setContentsMargins(6, 6, 6, 6)
        container_layout.addWidget(mw.properties_multi_select_container)

        mw.properties_edit_dialog_button = QPushButton("Advanced Edit...")
        container_layout.addWidget(mw.properties_edit_dialog_button)
