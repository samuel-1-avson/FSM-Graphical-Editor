...
from ..codegen import get_standard_icon, _get_bundled_file_path
from ..utils.config import (
    ...
    COLOR_BACKGROUND_MEDIUM, COLOR_BORDER_LIGHT, COLOR_TEXT_ON_ACCENT,
    COLOR_TEXT_PRIMARY
)
...
from ..ui.widgets.ribbon_toolbar import ProfessionalRibbon, ProfessionalGroup # <-- ADD RibbonGroup here
...
class UIManager(QObject): 
    ...
    def _create_docks(self):
        ...
        # --- NEW: Populate Data Dictionary Dock ---
        self._populate_data_dictionary_dock()
    ...
    # --- NEW: Method to populate data dictionary dock ---
    ...
    def _populate_elements_palette_dock(self):
        ...
        # Main widget for the entire dock
        dock_widget = QWidget()
        dock_layout = QVBoxLayout(dock_widget)
        dock_layout.setContentsMargins(4, 4, 4, 4)
        dock_layout.setSpacing(4)

        # Search bar at the top, outside the scroll area
        ...
        # Manage Assets button at the bottom of the dock, outside the scroll area
    ...
    def _load_and_display_templates(self):
        ...
        # --- FIX: Load icons directly from resource path, don't use helper ---
    ...
    def _populate_properties_dock(self):
        ...
        layout.setSpacing(6)
        # Multi-select container (initially hidden)
        mw.properties_multi_select_container = CollapsibleSection("Align & Distribute")
        mw.properties_multi_layout = QVBoxLayout() # We'll add buttons to this later
        ...
        # Button to open the more detailed properties dialog
        ...
        if 'config' in prop_info:
            for func_name, func_val in prop_info['config'].items():
                if func_name == "setRange" and isinstance(func_val, (list, tuple)):
                    # Unpack the tuple for methods that take multiple arguments
                    getattr(editor_widget, func_name)(*func_val)
                else:
                    # Call normally for methods with a single argument
                    getattr(editor_widget, func_name)(func_val)
        # --- END MODIFICATION ---
    ...
    def _commit_property_changes(self):
        ...
        This is typically triggered on FocusOut from an editor widget.
        """
        ...
        for item in self._current_edited_items:
            ...
            for key, editor_widget in self._property_editors.items():
                # Skip widgets that haven't been touched in a multi-edit scenario
    ...
    def _get_value_from_editor(self, editor_widget):
        if isinstance(editor_widget, QLineEdit): return editor_widget.text().strip()
            ...
            # For certain combo boxes, we need to return the lowercase version
