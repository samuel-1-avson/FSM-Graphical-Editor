...
class IDEManager(QObject):
    ...
    def populate_dock(self):
        """Creates the content for the IDE dock. Called after the dock widget itself exists."""
        ...
        lang_label.setStyleSheet(f"font-size: {config.APP_FONT_SIZE_SMALL}; padding-left: 3px; padding-right: 2px;")
        ...
        self.ide_code_editor.setStyleSheet(f"QPlainTextEdit#StandaloneCodeEditor {{ border-top: 1px solid {theme_config.COLOR_BORDER_LIGHT}; border-bottom: 1px solid {theme_config.COLOR_BORDER_LIGHT}; border-left: none; border-right: none; border-radius: 0px; }}")
        ...
        self.ide_output_console.setStyleSheet(f"QTextEdit#IDEOutputConsole {{ border-top: 1px solid {theme_config.COLOR_BORDER_DARK}; border-bottom: none; border-left: none; border-right: none; border-radius: 0px; }}")
    ...
    def _show_ide_context_menu(self, pos):
        menu = self.ide_code_editor.createStandardContextMenu()
        menu.addSeparator()
        
        if hasattr(self.mw, 'ide_analyze_selection_action'):
            self.mw.ide_analyze_selection_action.setEnabled(self.ide_code_editor.textCursor().hasSelection())
            menu.addAction(self.mw.ide_analyze_selection_action)
        
        menu.exec_(self.ide_code_editor.viewport().mapToGlobal(pos))
    ...
    def _connect_ide_actions(self):
        ...
        if hasattr(self.mw, 'ide_analyze_selection_action'):
            self.mw.ide_analyze_selection_action.triggered.connect(
                lambda: self.on_ide_analyze_with_ai(selection_only=True)
            )
    ...
    def on_ide_new_file(self):
        ...
        if old_path is not None: 
            self.ide_file_path_changed.emit("")
    ...
    def on_ide_run_python_script(self):
        ...
        if not code_to_run.strip():
            ...
            self.ide_output_console.append(f"<hr style='border-color:{theme_config.COLOR_BORDER_LIGHT};'><div style='color: {theme_config.COLOR_TEXT_SECONDARY}; font-size: {config.APP_FONT_SIZE_SMALL};'><i>Execution finished (no code).</i></div>")
            return

        self.ide_output_console.clear()
        self.ide_output_console.append(f"<div style='color: {theme_config.COLOR_TEXT_SECONDARY}; font-size: {config.APP_FONT_SIZE_SMALL};'><i>Running Python script at {QTime.currentTime().toString('hh:mm:ss')}...</i></div><hr style='border-color:{theme_config.COLOR_BORDER_LIGHT};'>")
        ...
        try:
            ...
            if err_output:
                error_color_hex = theme_config.COLOR_ACCENT_ERROR.name() if isinstance(theme_config.COLOR_ACCENT_ERROR, QColor) else theme_config.COLOR_ACCENT_ERROR
                self.ide_output_console.append(f"<pre style='color:{error_color_hex};'>{html.escape(err_output)}</pre>")
            self.ide_output_console.append(f"<hr style='border-color:{theme_config.COLOR_BORDER_LIGHT};'><div style='color: {theme_config.COLOR_TEXT_SECONDARY}; font-size: {config.APP_FONT_SIZE_SMALL};'><i>Execution finished.</i></div>")
        except Exception as e:
            import traceback
            error_color_hex = theme_config.COLOR_ACCENT_ERROR.name() if isinstance(theme_config.COLOR_ACCENT_ERROR, QColor) else theme_config.COLOR_ACCENT_ERROR
            ...
            self.ide_output_console.append(f"<hr style='border-color:{theme_config.COLOR_BORDER_LIGHT};'><div style='color: {error_color_hex}; font-size: {config.APP_FONT_SIZE_SMALL};'><i>Execution failed.</i></div>")
        finally:
    ...
    @pyqtSlot(bool)
    def on_ide_analyze_with_ai(self, selection_only=False):
        if not self.ide_code_editor or not self.ide_output_console:
            logger.error("IDE: Code editor or output console not available for AI analysis.")
            return
        if not self.mw.ai_chatbot_manager or not self.mw.ai_chatbot_manager.is_configured():
            QMessageBox.warning(self.mw, "AI Assistant Not Ready", "Please configure your AI provider and API key in AI Assistant Settings to use this feature.")
            return
        if not self.mw._internet_connected:
            QMessageBox.warning(self.mw, "AI Assistant Offline", "Internet connection is required for AI features.")
            return

        if selection_only:
            code_to_analyze = self.ide_code_editor.textCursor().selectedText()
            analysis_target_str = "the selected code snippet"
        else:
            code_to_analyze = self.ide_code_editor.toPlainText()
            analysis_target_str = "the current script"
        ...
        prompt = f"Please review {analysis_target_str} {language_context}. Check for syntax errors, common programming mistakes, potential bugs, or suggest improvements. Provide feedback and, if there are issues, offer a corrected version or explain the problem:\n\n```\n{code_to_analyze}\n```"

        self.ide_output_console.append(f"<div style='color: {theme_config.COLOR_TEXT_SECONDARY}; font-size: {config.APP_FONT_SIZE_SMALL};'><i>Sending code to AI for analysis ({selected_language})... (Response will appear in main AI Chat window)</i></div><hr style='border-color:{theme_config.COLOR_BORDER_LIGHT};'>")

        if self.mw.ai_chat_ui_manager:
            self.mw.ai_chat_ui_manager._append_to_chat_display("IDE", f"Requesting AI analysis for {analysis_target_str} ({selected_language}).")
