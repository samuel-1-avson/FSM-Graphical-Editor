...
class CCompilerWorker(QObject):
    ...
    def _find_compiler(self) -> (str, List[str]):
        ...
        # --- FIX: Apply the environment fix to the compiler detection call ---
        try:
            # Pass the full environment and set shell=False for robustness.
            subprocess.run(['cl', '/?'], capture_output=True, check=True, timeout=5, shell=False, env=os.environ.copy())
