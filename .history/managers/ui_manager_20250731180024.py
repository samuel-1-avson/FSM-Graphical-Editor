...
class UIManager(QObject): 
    ...
    def _display_multi_item_view(self, items):
        ...
        if all(isinstance(i, item_type) for i in items):
            ...
            self._build_common_editors(items)
        else:
    ...
    def _build_common_editors(self, items):
                ...
                if is_multi_edit:
                    for other_item in items_list[1:]:
                        if other_item.get_data().get(key) != first_value:
                            all_same = False
                            break
                editor_widget = self._create_editor_widget(prop_info, first_value, all_same or is_multi_edit)
    ...
    def _create_editor_widget(self, prop_info, value, is_single_edit_or_all_same):
        key = prop_info['key']
        WidgetClass = prop_info['widget']
        editor_widget = WidgetClass()
        if is_single_edit_or_all_same:
            ...
            elif isinstance(editor_widget, QDoubleSpinBox): editor_widget.setValue(float(value or 0))
        else:
        ...
        if prop_info.get('is_color') and isinstance(editor_widget, QPushButton):
            color = QColor(value) if is_single_edit_or_all_same and value else QColor(Qt.gray)
