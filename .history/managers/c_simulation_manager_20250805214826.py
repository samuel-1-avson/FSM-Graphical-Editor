        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if sys.platform == "win32":
                    lib_ext = ".dll"
                    compiler_cmd = ['gcc', '-shared', '-o']
                elif sys.platform == "darwin":
                    lib_ext = ".dylib"
                    compiler_cmd = ['gcc', '-shared', '-o']
                else: # Linux
                    lib_ext = ".so"
                    compiler_cmd = ['gcc', '-shared', '-fPIC', '-o']
                
                lib_path = os.path.join(temp_dir, f"{fsm_name}{lib_ext}")
                command = compiler_cmd + [lib_path, c_path]
                ...
                if process.returncode == 0:
                    # On success, we must copy the library out of the temporary dir
                    # before the context manager deletes it.
                    ...
                    import shutil
                    shutil.copy(lib_path, final_lib_path)
            ...
            self.compile_finished.emit(False, "", "Compiler 'gcc' not found. Is it installed and in your system's PATH?")
