...
class UIManager(QObject): 
    ...
    def _create_actions(self):
        ...
        mw.open_action = QAction(get_standard_icon(QStyle.SP_DialogOpenButton, "Opn"), "&Open Project...", mw, shortcut=QKeySequence.Open, statusTip="Open an existing project or file")
        ...
        mw.save_as_action.setEnabled(False) # Disable initially
        mw.close_project_action = QAction("Close Project", mw, statusTip="Close the current project", enabled=False)
    ...
    def _create_docks(self):
        ...
        docks_to_create = {
            ...
            "live_preview_dock": ("LivePreviewDock", "Code Scratchpad"), # RENAMED
    ...
    # --- MODIFIED: This is now the definitive, project-aware implementation ---
    def _populate_project_explorer_dock(self):
        """Creates and configures the content for the Project Explorer dock."""
        mw = self.mw
        
        # This model is now initialized in MainWindow, as it's part of the app state
        mw.project_fs_model = QFileSystemModel()
        mw.project_fs_model.setRootPath(QDir.homePath()) # Default to home until a project is loaded
        
        # Define filters to hide clutter and irrelevant files
        hidden_items = [".git", ".venv", "__pycache__", ".vscode", "*.pyc"]
        mw.project_fs_model.setNameFilters(hidden_items)
        mw.project_fs_model.setNameFilterDisables(True) # True means hide, not just grey out
        ...
        # The signal will be connected in MainWindow
        # --- END NEW ---
    ...
    def _populate_live_preview_dock(self):
        ...
        # Only include formats that we can reliably parse back into a diagram
        mw.live_preview_combo.addItems(["PlantUML", "Mermaid", "Python FSM"])
        toolbar.addWidget(mw.live_preview_combo)
        
        toolbar.addSeparator()

        # --- NEW: Add Sync and Revert Buttons ---
        ...
        mw.scratchpad_sync_action.setEnabled(False) # Disabled by default
        # --- END NEW ---
        
        layout.addWidget(toolbar)
        
        mw.live_preview_editor = CodeEditor()
        # --- MODIFICATION: Make the editor EDITABLE ---
    ...
    def update_save_actions_state(self):
        ...
        if hasattr(mw, 'save_as_action'):
            # --- MODIFIED: Enable Save As only if a project is open and a file is active ---
