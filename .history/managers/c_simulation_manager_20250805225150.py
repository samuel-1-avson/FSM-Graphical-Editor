...
from PyQt5.QtWidgets import (
    ...
    QStyle, QMessageBox, QHBoxLayout, QLineEdit, QLabel
)
from PyQt5.QtCore import QObject, pyqtSlot, pyqtSignal, QThread, QMetaObject, Q_ARG, Qt, QDir
...
class CCompilerWorker(QObject):
    ...
    def run_compile(self, c_code: str, h_code: str, fsm_name: str):
        """
        Compiles the given C code into a shared library in a temporary directory.
        """
        try:
            compiler_name, compiler_base_cmd = self._find_compiler()

            with tempfile.TemporaryDirectory() as temp_dir:
                c_path = os.path.join(temp_dir, f"{fsm_name}.c")
                h_path = os.path.join(temp_dir, f"{fsm_name}.h")
                ...
                lib_path_temp = os.path.join(temp_dir, f"{fsm_name}{lib_ext}")

                if compiler_name == 'cl':
                    command = compiler_base_cmd + [lib_path_temp, c_path]
                else: # gcc
                    command = compiler_base_cmd + [lib_path_temp, c_path]

                logger.info(f"Running compiler command: {' '.join(command)}")
                
                # On Windows, we might need to run this in a shell to pick up the VS environment
                ...
                if process.returncode == 0:
                    final_lib_dir = os.path.join(tempfile.gettempdir(), f"bsm_c_sim_{os.getpid()}")
                    os.makedirs(final_lib_dir, exist_ok=True)
                    final_lib_path = os.path.join(final_lib_dir, f"{fsm_name}{lib_ext}")
                    
                    shutil.copy(lib_path_temp, final_lib_path)
                    
                    # MSVC also creates .lib and .exp files we should clean up or copy if needed.
                    # For ctypes, we only need the DLL, so we can ignore them for now.

                    self.compile_finished.emit(True, final_lib_path, output)
                else:
