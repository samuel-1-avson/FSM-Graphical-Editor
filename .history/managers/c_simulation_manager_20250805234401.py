--- START OF FILE fsm_designer_project/managers/c_simulation_manager.py ---
...
class CCompilerWorker(QObject):
    ...
    def _find_compiler(self) -> (str, List[str]):
        ...
        try:
            # Check if 'cl' is in the environment path passed to this process
            subprocess.run(['cl', '/?'], capture_output=True, check=True, timeout=5, shell=True)
            logger.info("Found MSVC compiler (cl.exe).")
            return 'cl', ['cl', '/LD', '/W3', '/D', 'FSM_CORE_BUILD_DLL', '/Fe:']
    ...
    def run_compile(self, c_code: str, h_code: str, fsm_name: str):
        ...
        try:
            ...
            # --- FIX: Explicitly pass the full environment ---
            # Capture the current environment, which will include the VS dev vars
            env = os.environ.copy()
            ...
            # This is the crucial change. We pass the full environment from the
            # Developer Command Prompt to the subprocess, and we set shell=False
            # for better security and argument handling.
            process = subprocess.run(
                ...
                encoding='utf-8',
                errors='replace',
                shell=False,  # Set to False for robustness
                cwd=build_dir,
                env=env # Pass the captured environment
            )
            ...
            self.compile_finished.emit(False, "", f"Compiler command '{compiler_base_cmd[0]}' not found. Please install a C compiler and ensure it's in your system's PATH, or run this application from a Developer Command Prompt for Visual Studio.")
...
# --- Manager and UI class (No changes from here down) ---
...
--- END OF FILE fsm_designer_project/managers/c_simulation_manager.py ---