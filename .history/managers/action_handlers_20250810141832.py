...
# --- MODIFIED: Import from new packages ---
from ..utils import get_standard_icon, _get_bundled_file_path
from ..utils.config import FILE_FILTER, FILE_EXTENSION, DEFAULT_EXECUTION_ENV, PROJECT_FILE_FILTER, PROJECT_FILE_EXTENSION
...
class ActionHandler(QObject):
    ...
    def on_export_c_testbench(self):
        ...
        # --- MODIFIED: Import from new codegen package ---
        from ..codegen.c_code_generator import sanitize_c_identifier, generate_c_testbench_content
    ...
    def on_export_vhdl(self):
        ...
        if editor.file_path:
            base_name = os.path.splitext(os.path.basename(editor.file_path))[0]
            from ..codegen.hdl_code_generator import sanitize_vhdl_identifier
        ...
        diagram_data = editor.scene.get_diagram_data()
        try:
            from ..codegen.hdl_code_generator import generate_vhdl_content, sanitize_vhdl_identifier
    ...
    def on_export_verilog(self):
        ...
        if editor.file_path:
            base_name = os.path.splitext(os.path.basename(editor.file_path))[0]
            from ..codegen.hdl_code_generator import sanitize_vhdl_identifier # Verilog sanitizer is similar
        ...
        diagram_data = editor.scene.get_diagram_data()
        try:
            from ..codegen.hdl_code_generator import generate_verilog_content
