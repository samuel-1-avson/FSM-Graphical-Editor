...
from PyQt5.QtWidgets import (
    ...
    QMessageBox, QInputDialog, QLineEdit, QSizePolicy, QTreeView, QFileSystemModel
)
from PyQt5.QtGui import QIcon, QKeySequence, QPalette, QPainter, QColor, QFont
from PyQt5.QtCore import Qt, QSize, QObject, QPointF, pyqtSlot, QDir

from ..utils import get_standard_icon, _get_bundled_file_path
from ..ui.widgets.custom_widgets import DraggableToolButton
# --- START OF FIX ---
# FSM_TEMPLATES_BUILTIN was moved from config.py to assets.py.
# This import is being corrected.
...
from ..ui.widgets.custom_widgets import CollapsibleSection
...
class UIManager(QObject): 
    ...
    def _create_actions(self):
        ...
        # --- FILE ACTIONS ARE NOW PROJECT ACTIONS ---
        mw.new_action = QAction(get_standard_icon(QStyle.SP_FileIcon, "New"), "&New Project...", mw, shortcut=QKeySequence.New, statusTip="Create a new project")
        mw.open_action = QAction(get_standard_icon(QStyle.SP_DialogOpenButton, "Opn"), "&Open Project...", mw, shortcut=QKeySequence.Open, statusTip="Open an existing project")
        mw.save_action = QAction(get_standard_icon(QStyle.SP_DialogSaveButton, "Sav"), "&Save Project", mw, shortcut=QKeySequence.Save, statusTip="Save the current project", enabled=False)
        mw.save_as_action = QAction(get_standard_icon(self._safe_get_style_enum("SP_DriveHDIcon", "SP_DialogSaveButton"), "SA"), "Save Project &As...", mw, shortcut=QKeySequence.SaveAs, statusTip="Save the current project with a new name")
        mw.close_project_action = QAction("Close Project", mw, statusTip="Close the current project")
    ...
    def _create_ribbon(self):
        mw = self.mw
        # Remove any existing toolbars and the menubar
        ...
        # --- Setup the Menus that will be used by the Ribbon's buttons ---
        mw.view_menu = QMenu("&View", mw)
        mw.toolbars_menu = mw.view_menu.addMenu("Toolbars") # This menu is now for showing/hiding the ribbon itself
        ...
        file_menu.addAction(mw.close_project_action)
        
        export_menu = file_menu.addMenu("E&xport")
        export_image_menu = export_menu.addMenu("Image")
        export_image_menu.addActions([mw.export_png_action, mw.export_svg_action])
        export_doc_menu = export_menu.addMenu("Documentation")
        export_doc_menu.addActions([mw.export_plantuml_action, mw.export_mermaid_action])
        export_code_menu = export_menu.addMenu("Code")
        
        # --- NEW: Add dedicated Arduino export action ---
        mw.export_arduino_action = QAction("Arduino Sketch (.ino)...", mw)
        export_code_menu.addAction(mw.export_arduino_action)
        
        export_code_menu.addActions([mw.generate_c_code_action, mw.export_c_testbench_action, mw.export_python_fsm_action])
        export_hdl_menu = export_menu.addMenu("HDL")
        export_hdl_menu.addActions([mw.export_vhdl_action, mw.export_verilog_action])
        export_menu.addSeparator()
        export_menu.addAction(mw.export_simulink_action)
        
        # Add plugin actions to the export menu
        if hasattr(mw, 'plugin_manager') and mw.plugin_manager.exporter_plugins:
            export_menu.addSeparator()
            for plugin in mw.plugin_manager.exporter_plugins:
                plugin_action = export_menu.addAction(f"as {plugin.name}...")
                plugin_action.triggered.connect(lambda checked=False, p=plugin: mw.action_handler.on_export_with_plugin(p))
        ...
        editing_group.add_action_button(mw.manage_snippets_action, is_large=False)
        ...
        align_btn = QToolButton(); align_btn.setText("Align"); align_btn.setPopupMode(QToolButton.InstantPopup); align_btn.setMinimumSize(70, 28)
        align_menu = QMenu(mw); align_menu.addActions(mw.align_actions); align_btn.setMenu(align_menu)
        dist_btn = QToolButton(); dist_btn.setText("Distribute"); dist_btn.setPopupMode(QToolButton.InstantPopup); dist_btn.setMinimumSize(70, 28)
    ...
    def _create_docks(self):
        ...
        # --- FIX: Set corner behavior to prevent bottom docks from underlapping right docks ---
    ...
    def _populate_project_explorer_dock(self):
        """Creates the content for the Project Explorer dock."""
        mw = self.mw
        
        # Filesystem model to display directory contents
        mw.project_fs_model = QFileSystemModel()
        mw.project_fs_model.setRootPath(QDir.homePath()) # Default to home
        mw.project_fs_model.setFilter(QDir.NoDotAndDotDot | QDir.AllDirs | QDir.Files)

        # Tree view to display the model
    ...
    # --- NEW: Slot for filtering the properties dock ---
    @pyqtSlot(str)
    def _on_filter_properties_dock(self, text: str):
        """Filters the visible properties based on the search text."""
        filter_text = text.lower().strip()
        mw = self.mw

        # Iterate through the collapsible sections (e.g., "General", "Appearance")
        for i in range(mw.properties_editor_layout.count()):
            ...
            # Iterate through all widgets in the section's content layout
            for row_index in range(content_layout.count()):
                ...
                # The label is the first widget in the row's QHBoxLayout
                row_layout = row_widget.layout()
                if row_layout and row_layout.count() > 0:
                    label_widget = row_layout.itemAt(0).widget()
                    if isinstance(label_widget, QLabel):
                        # A row matches if the filter text is in its label OR in the section title
                ...
                elif isinstance(row_widget, QCheckBox): # Handle rows that are just a checkbox
                    matches = not filter_text or filter_text in row_widget.text().lower()
                    row_widget.setVisible(matches)
                    if matches:
                        section_visible_due_to_match = True

            # A section is visible if it matches, OR if the filter is empty (show all)
