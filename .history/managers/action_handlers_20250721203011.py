...
from PyQt5.QtWidgets import QGroupBox # Add QGroupBox
...
class ActionHandler(QObject):
    def __init__(self, main_window):
        ...
        # --- NEW: Central list of all actions that should be disabled during simulation ---
    ...
    def on_new_file(self, silent=False):
        ...
        if hasattr(self.mw, 'statusBar') and self.mw.statusBar() and hasattr(self.mw.statusBar(), 'set_status'):
            self.mw.statusBar().set_status("New diagram. Ready.", "success", 3000)
        else:
    ...
    def on_open_file(self):
        ...
        # --- MODIFICATION: Build dynamic file filter from plugins ---
        filters = [FILE_FILTER]
        ...
        file_paths, selected_filter = QFileDialog.getOpenFileNames(self.mw, "Open File(s)", start_dir, file_filter_str)
        ...
        if file_paths:
            for file_path in file_paths:
                ...
                # --- MODIFICATION: Check if a plugin should be used for import ---
                if FILE_FILTER in selected_filter or file_path.endswith(FILE_EXTENSION):
                    # This is a native .bsm file, load it normally
                    self.mw._create_and_load_new_tab(file_path)
                else:
                    # Try to find a plugin that matches the selected filter
                    importer_plugin = None
                    if hasattr(self.mw, 'plugin_manager'):
                        for plugin in self.mw.plugin_manager.importer_plugins:
                            if plugin.file_filter == selected_filter:
                    ...
                    if importer_plugin:
                        ...
                        # Fallback for "All Files" - check by extension
                        importer_plugin = self._find_importer_by_extension(file_path)
                        if importer_plugin:
                            self._import_with_plugin(file_path, importer_plugin)
                        else:
                            QMessageBox.warning(self.mw, "Unsupported File", f"No importer found for the selected file type: {os.path.basename(file_path)}")
                # --- END MODIFICATION ---
