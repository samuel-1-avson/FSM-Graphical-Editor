    ...
    def update_dock_ui_contents(self):
        ...
        if not editor or not editor.py_fsm_engine or not editor.py_sim_active: 
            ...
            if hasattr(self.mw, 'hardware_sim_ui_manager'):
                ...
                # If HIL is active, send commands to turn off all mapped components
                if hw_mgr.hardware_link_manager.is_connected:
                    # This could be improved by tracking which components were used
        ...
        # --- UPDATED: Hardware Mapping Logic ---
        if hasattr(self.mw, 'hardware_sim_ui_manager'):
            hw_mgr = self.mw.hardware_sim_ui_manager
            # Find all hardware output mappings in the current state
            output_mappings = {}
            if editor.py_fsm_engine.current_state_path:
                current_leaf_state_name = editor.py_fsm_engine.get_current_leaf_state_name()
                for item in editor.scene.items():
                    if isinstance(item, GraphicsStateItem) and item.text_label == current_leaf_state_name:
                        d = item.get_data()
                        if d.get('hw_output_variable') and d.get('hw_component_map') != "None":
                            output_mappings[d['hw_output_variable']] = d.get('hw_component_map')
                        break

            sim_variables = editor.py_fsm_engine.get_variables()
            
            # Iterate through all known virtual components to update them
            ...
            for comp_key, comp_widget in all_components.items():
                ...
                # Check if this component is mapped in the current state
                for var, comp in output_mappings.items():
                    if comp == comp_key:
                        mapped_var_name = var
                        break
                
                new_value = 0 # Default to off/zero if not mapped in this state
                if mapped_var_name and mapped_var_name in sim_variables:
                    new_value = sim_variables[mapped_var_name]

                # Update virtual UI and send command to physical hardware
