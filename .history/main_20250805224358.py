Excellent. We have now laid all the groundwork for the C simulation engine. The C++ core is designed, the Python wrapper is ready, and the UI knows where the new dock should go.

The final step is to bring it all together inside `MainWindow`. This is where we will instantiate the `CSimulationManager` and connect its signals to update the rest of the application.

Here are the final modifications needed for `fsm_designer_project/main.py`.

---

#### **Final Code: Modifying `MainWindow`**

We will make the following changes to `main.py`:
1.  **Import `CSimulationManager`** alongside the other managers.
2.  **Instantiate** it in the `__init__` method.
3.  **Connect its `simulationStateChanged` signal** to a new handler method that will enable/disable other UI elements (like the Python simulator and diagram editing tools) to prevent conflicts.

This will complete the integration of the C++ simulation engine.

```python
--- START OF FILE fsm_designer_project/main.py ---
...
import html
import jsonschema  # <-- NEW: Import for schema validation
...
# --- MODIFICATION: Import CSimulationManager ---
from .managers import MatlabSimulationManager, SettingsManager, CSimulationManager
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 1. Core managers with no dependencies on other managers
        ...
        self.matlab_connection = MatlabConnection() # Starts lazily now
        ...
        # 2. Managers that depend on core managers
        self.action_handler = ActionHandler(self) # Depends on MainWindow for UI access
        self.ui_manager = UIManager(self) # Depends on MainWindow for UI access
        ...
        # 3. UI and Feature managers that depend on multiple other managers
        ...
        # --- NEW: Assign C Simulation Manager from UIManager ---
        ...
        # Populate dock contents now that all managers exist
    ...
    def _update_central_widget(self):
        ...
        if self.tab_widget.count() == 0:
            ...
            # --- MODIFICATION: Explicitly update the list whenever the welcome screen is shown ---
    ...
    def _connect_application_signals(self):
        ...
        # --- NEW: Connect C Simulation Manager Signal ---
        if self.c_sim_manager:
            self.c_sim_manager.simulationStateChanged.connect(self._handle_c_sim_state_changed)
    ...
    def _on_close_tab_requested(self, index: int):
        ...
        # --- FIX: Explicitly disconnect signals from the editor being closed ---
        # This prevents dangling connections from firing during object destruction.
        try:
    ...
    def _on_current_tab_changed(self, index: int):
        # --- Disconnect from ALL editors first to be safe ---
        for i in range(self.tab_widget.count()):
            if i == index: continue # Skip the current tab
            ...
            if isinstance(editor_to_disconnect, EditorWidget) and hasattr(self, 'undo_action'):
                try:
                    ...
                    editor_to_disconnect.undo_stack.undoTextChanged.disconnect() # Disconnect all slots from this signal
                    editor_to_disconnect.undo_stack.redoTextChanged.disconnect() # Disconnect all slots from this signal
                except (TypeError, RuntimeError):
                    pass # Ignore errors if signals were not connected

        editor = self.current_editor()
        if editor and hasattr(self, 'undo_action'):
            # Connect to the new editor's undo stack
            ...
            # --- Initialize the state correctly for the new tab ---
        ...
        elif hasattr(self, 'undo_action'): # Handle case where there are no editors left
    ...
    def _update_matlab_actions_enabled_state(self):
        ...
        # --- FIXED: Changed .connected to .is_connected() ---
    ...
    def _handle_matlab_modelgen_or_sim_finished(self, success, message, data):
        ...
        if success:
            if "Model generation" in message and data:
                self.last_generated_model_path = data # <-- MODIFIED
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        # --- If we get here, the app is closing for sure. Now do the cleanup. ---
 
        # --- FIX: Disconnect all editor signals before shutdown to prevent crashes ---
        # This is the crucial step. We sever all connections from the editor undo stacks
        # to the main window's actions before Qt starts destroying objects.
        for i in range(self.tab_widget.count()):
            editor = self.tab_widget.widget(i)
            if isinstance(editor, EditorWidget):
                try:
                    ...
                    # This is expected and harmless during shutdown if signals are already disconnected.
                    pass
 
        # --- FIXED: Cleanly remove the UI log handler from the logging system ---
        if hasattr(self, 'ui_log_handler'):
            # This is the crucial part for the new fix.
            # We remove it from the root logger's list.
            logging.getLogger().removeHandler(self.ui_log_handler)
            # We also try to remove it from the global handler list that atexit uses.
            # This uses a private function, which is not ideal but necessary.
            if hasattr(logging, '_removeHandlerRef'):
                logging._removeHandlerRef(self.ui_log_handler)
            self.ui_log_handler.close() # This is good practice for logging handlers
            logger.info("MW_CLOSE: UI log handler removed from root logger and global list.")
        # --- END FIX ---
        ...
        # --- NEW: Add shutdown for CSimulationManager ---
        ...
        # --- NEW: Gracefully shut down the MATLAB engine ---
        if self.matlab_connection: self.matlab_connection.shutdown()
        
        if self.matlab_sim_manager: self.matlab_sim_manager.shutdown() # <-- NEW
...
if __name__ == '__main__':
    main_entry_point()
--- END OF FILE fsm_designer_project/main.py ---
```
...
*   **`managers/ui_manager.py`:**
    *   Imported the `CSimulationManager`.
    *   Instantiated it in the `__init__` method.
    *   Added `c_sim_dock` to the list of docks to create.
    *   In `populate_dynamic_docks`, added the logic to create and set the widget for the C simulation dock.
    *   Crucially, in `_create_docks`, I've tabified the Python and C simulation docks together, so they appear neatly in the same area on the right: `mw.tabifyDockWidget(mw.py_sim_dock, mw.c_sim_dock)`.

*   **`main.py`:**
    *   Imported the `CSimulationManager`.
    *   In `__init__`, assigned `self.c_sim_manager` by getting the instance from the already-initialized `UIManager`.
    *   In `_connect_application_signals`, I connected the `simulationStateChanged` signal from the new `c_sim_manager` to a handler slot.
    *   Added the new slot `_handle_c_sim_state_changed` which is responsible for disabling other UI elements (like the Python sim buttons and diagram editing) whenever the C simulation is active, preventing conflicts.
    *   Added the `c_sim_manager.shutdown()` call to the main window's `closeEvent` to ensure the background compiler thread is terminated cleanly when the application exits.

### What to Do Next

1.  **Apply the code changes** to `fsm_designer_project/managers/ui_manager.py` and `fsm_designer_project/main.py`.
2.  **Ensure you have a C compiler** installed and available in your PATH (or run the app from a Developer Command Prompt for Visual Studio).
3.  **Re-compile the C++ core engine** if you haven't already:
    ```bash
    cd fsm_designer_project/core_engine
...
You will now see a "C Simulation" tab next to the "Python Simulation" dock. You can now:
*   Create a diagram.
*   Click "Compile FSM". Observe the compiler output in the dock.
*   If successful, click "Initialize" to load the FSM into the C engine.
*   Use the "Trigger" button to send events and watch the state change, all running on the compiled C++ backend.