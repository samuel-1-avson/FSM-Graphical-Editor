...
from PyQt5.QtGui import QIcon, QKeySequence, QPalette, QPainter
...
class UIManager(QObject): 
    ...
    def _populate_properties_dock(self):
        ...
        # Sub-FSM Previewer
        mw.sub_fsm_preview_group = QGroupBox("Sub-Machine Preview")
        preview_layout = QVBoxLayout(mw.sub_fsm_preview_group)
        mw.sub_fsm_preview_view = QGraphicsView()
        mw.sub_fsm_preview_view.setRenderHints(QPainter.Antialiasing | QPainter.SmoothPixmapTransform)
        mw.sub_fsm_preview_view.setMinimumHeight(120)
        mw.sub_fsm_preview_view.setInteractive(False)
        preview_layout.addWidget(mw.sub_fsm_preview_view)
        layout.addWidget(mw.sub_fsm_preview_group)
        mw.sub_fsm_preview_group.setVisible(False)
