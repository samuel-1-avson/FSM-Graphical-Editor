...
class ResourceMonitorWorker(QObject):
    ...
    def stop_monitoring(self):
        logger.info("ResourceMonitorWorker: stop_monitoring slot called. Setting _stop_requested = True.")
    ...
    def _monitor_resources(self):
        ...
        while not self._stop_requested:
            ...
            # This sleep is crucial. It yields control to the thread's event loop
            # so that the 'stop_monitoring' slot can be processed.
...
class ResourceMonitorManager(QObject):
    ...
    def stop_monitoring_system(self):
        ...
        if self.thread and self.thread.isRunning():
            ...
            if self.worker:
                # Post the stop request to the worker's event queue.
                QMetaObject.invokeMethod(self.worker, "stop_monitoring", Qt.QueuedConnection)
                logger.debug("ResourceMonitorManager: stop_monitoring slot queued on resource worker.")
            else:
                 # Fallback if worker is gone but thread is somehow running
                 self.thread.quit()
