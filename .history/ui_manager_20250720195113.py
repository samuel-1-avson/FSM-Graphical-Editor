...
class UIManager(QObject): 
    ...
    def setup_ui(self):
        ...
        self._create_menus()
        self._create_toolbars() # This will now create the ribbon
        self._create_docks()
    ...
    def _create_menus(self):
        ...
        # --- FIX: Ensure recent_files_menu exists before connecting ---
        if not hasattr(mw, 'recent_files_menu'):
        ...
        c_export_menu = export_menu.addMenu("C/C++ Export")
        c_export_menu.addAction(mw.generate_c_code_action)
        c_export_menu.addAction(mw.export_c_testbench_action)
        ...
        assets_menu = edit_menu.addMenu("Manage Assets")
        edit_menu.addMenu("Interaction Mode").addActions(mw.mode_action_group.actions())
        assets_menu.addAction(mw.manage_snippets_action); assets_menu.addAction(mw.manage_fsm_templates_action)
        ...
        log_menu = edit_menu.addMenu("Log"); # NEW Log Menu
        log_menu.addActions([mw.log_clear_action, mw.log_copy_action, mw.log_save_action])
    ...
    def _create_toolbars(self):
        """Creates the new ribbon-style toolbar."""
        mw = self.mw
        ...
        # Create the main ribbon widget and a toolbar to contain it
        ...
        # Make the ribbon toolbar toggleable from the View menu
        mw.toolbars_menu.clear()
        mw.toolbars_menu.addAction(ribbon_container_toolbar.toggleViewAction())

        # --- Populate Ribbon Tabs and Groups ---
        # File Tab
        file_tab = mw.ribbon.add_tab("File")
        file_group = RibbonGroup("File")
        
        new_btn = RibbonButton("New", mw.new_action.icon())
        new_btn.clicked.connect(mw.new_action.trigger)
        file_group.add_widget(new_btn)
        
        open_btn = RibbonButton("Open", mw.open_action.icon())
        open_btn.clicked.connect(mw.open_action.trigger)
        file_group.add_widget(open_btn)
        
        save_btn = RibbonButton("Save", mw.save_action.icon())
        save_btn.clicked.connect(mw.save_action.trigger)
        file_group.add_widget(save_btn)
        
        file_tab.add_group(file_group)

        # Edit Tab
        edit_tab = mw.ribbon.add_tab("Edit")
        history_group = RibbonGroup("History")

        undo_btn = RibbonButton("Undo", mw.undo_action.icon())
        undo_btn.clicked.connect(mw.undo_action.trigger)
        history_group.add_widget(undo_btn)
        
        redo_btn = RibbonButton("Redo", mw.redo_action.icon())
        redo_btn.clicked.connect(mw.redo_action.trigger)
        history_group.add_widget(redo_btn)
        
        edit_tab.add_group(history_group)

        # Design Tab
        design_tab = mw.ribbon.add_tab("Design")
        mode_group = RibbonGroup("Mode")
        for action in mw.mode_action_group.actions():
            btn = RibbonButton(action.text(), action.icon())
            btn.setCheckable(True)
            btn.clicked.connect(action.trigger)
            action.toggled.connect(btn.setChecked)
            mode_group.add_widget(btn)
        design_tab.add_group(mode_group)

        layout_group = RibbonGroup("Layout")
        auto_layout_btn = RibbonButton("Auto-Layout", mw.auto_layout_action.icon())
        auto_layout_btn.clicked.connect(mw.auto_layout_action.trigger)
        layout_group.add_widget(auto_layout_btn)
        design_tab.add_group(layout_group)
    ...
    def _create_docks(self):
        ...
        # --- NEW: Re-architect log dock setup ---
        ...
        log_layout.addWidget(mw.log_output, 1) # Give the text edit stretching priority
        mw.log_dock.setWidget(log_widget_container)
        # --- END NEW LOG DOCK SETUP ---
