...
from .dialogs import (MatlabSettingsDialog, FindItemDialog,SettingsDialog, SnippetManagerDialog)
...
from .editor_widget import EditorWidget # <-- Add this new import
...
class MainWindow(QMainWindow):
    ...
    def _update_properties_dock(self):
        ...
        # Clear existing layout and data
            ...
            if hasattr(self._current_edited_item_in_dock, 'get_data'):
                ...
                if schema:
                    ...
                    for prop_info in schema:
                        ...
                        editor = WidgetClass()
                        if isinstance(editor, QLineEdit): editor.setText(str(current_value or ''))
                        elif isinstance(editor, QTextEdit): editor.setPlainText(str(current_value or ''))
                        elif isinstance(editor, QCheckBox): editor.setChecked(bool(current_value))
                        elif isinstance(editor, QSpinBox): editor.setValue(int(current_value or 0))
                        elif isinstance(editor, QDoubleSpinBox): editor.setValue(float(current_value or 0))
                        
                        # --- BUG FIX: Rename local var from 'config' to 'widget_config' ---
                        if widget_config := prop_info.get('config'):
                            for method_name, args in widget_config.items():
                                if hasattr(editor, method_name):
                                    if isinstance(args, tuple): getattr(editor, method_name)(*args)
                                    else: getattr(editor, method_name)(args)
                        # --- END BUG FIX ---

                        # Handle special cases like the color button
                        if prop_info.get('is_color', False) and isinstance(editor, QPushButton):
                            editor.setObjectName("ColorButtonPropertiesDock")
                            ...
                            editor.clicked.connect(lambda ch, btn=editor: self._on_dock_color_button_clicked(btn))

                        # Add to layout
                        if isinstance(editor, QCheckBox): self.properties_editor_layout.addRow(editor)
                        else: self.properties_editor_layout.addRow(label, editor)
                        
                        # Add signal connection and store reference
                        if hasattr(editor, 'toggled'): editor.toggled.connect(self._on_dock_property_changed_mw)
                        elif hasattr(editor, 'textChanged'): editor.textChanged.connect(self._on_dock_property_changed_mw)
                        elif hasattr(editor, 'valueChanged'): editor.valueChanged.connect(self._on_dock_property_changed_mw)
                        self._dock_property_editors[key] = editor
                else: # Item type has no schema
        ...
        # Update visibility and button states based on selection
    ...
    def __init__(self):
        ...
        # The central widget is now a QTabWidget
        self.tab_widget = QTabWidget()
        self.tab_widget.setTabsClosable(True)
        self.tab_widget.setMovable(True)
        self.tab_widget.setDocumentMode(True) # Use a style suitable for documents
        self.tab_widget.tabCloseRequested.connect(self._on_close_tab_requested)
        self.tab_widget.currentChanged.connect(self._on_current_tab_changed)
        self.setCentralWidget(self.tab_widget)
        # 2. Initialize scene and view (central widget)
        ...
        # 3. Instantiate all manager classes BEFORE using their methods or setting them up in the UI
        ...
        # This populates self with attributes like self.log_output, self.ide_dock etc.
        ...
        # 6. Setup Logging, which requires the log_output widget created in setup_ui
        try:
        ...
        if self.ai_chat_ui_manager and self.ai_chatbot_manager:
            ...
            logger.warning("MainWindow: AI managers not fully initialized for final status update.")
       
       
       
    # --- NEW Method for Recent Files Menu ---
    def _populate_recent_files_menu(self):
        # Implementation is in UIManager, which calls this method on MainWindow
        # It's kept here because it modifies the menu owned by MainWindow
        if hasattr(self, 'ui_manager') and hasattr(self.ui_manager, 'recent_files_menu'):
            menu = self.ui_manager.recent_files_menu
            menu.clear()
            ...
            for i, file_path in enumerate(recent_files):
                ...
                action = QAction(action_text, self, triggered=self.action_handler.on_open_recent_file)
                action.setData(file_path); action.setToolTip(file_path)
                menu.addAction(action)

            menu.addSeparator()
            menu.addAction("Clear List", self._clear_recent_files_list)
        else:
            logger.warning("Recent files menu not found, cannot populate.")
    ...
    def _connect_signals(self): 
        ...
        self.scene.interaction_mode_changed.connect(self._on_interaction_mode_changed_by_scene) 
        self.scene.sceneRectChanged.connect(lambda _: self.update_resource_estimation())  
        self.settings_manager.settingChanged.connect(self._handle_setting_changed)
        self.scene.item_moved.connect(self._update_item_properties_from_move) # New connection to update dock on move
    ...
    def add_editor_tab(self, editor_widget: EditorWidget, title: str) -> int:
        """Adds a new editor widget as a tab."""
        index = self.tab_widget.addTab(editor_widget, title)
        self.tab_widget.setCurrentIndex(index)
        editor_widget.view.setFocus()
        return index
    ...
    def _create_and_load_new_tab(self, file_path: str):
        ...
        if self._load_into_editor(new_editor, file_path):
            ...
            self.log_message("INFO", f"Opened file: {file_path}")
        else:
    ...
    def _connect_editor_signals(self, editor: EditorWidget):
        ...
        # Main undo/redo actions are connected/disconnected dynamically in _on_current_tab_changed
        editor.scene.selectionChanged.connect(self._update_ui_element_states)
        editor.scene.scene_content_changed_for_find.connect(self._refresh_find_dialog_if_visible)
        editor.scene.modifiedStatusChanged.connect(self._update_window_title) # Title includes dirty state
    ...
    def _on_close_tab_requested(self, index: int):
        ...
        if not editor or not self._prompt_save_on_close(editor): return
        ...
        if self.tab_widget.count() == 0:
            self.action_handler.on_new_file() # Ensure there's always a tab
    ...
    def _on_current_tab_changed(self, index: int):
        ...
        if editor and editor.undo_stack:
    ...
    def _load_into_editor(self, editor: EditorWidget, file_path: str) -> bool:
        # Implementation moved to here from ActionHandler to centralize logic
        try:
            data = None
    ...
    def _save_editor_to_path(self, editor: EditorWidget, file_path: str) -> bool:
        # Implementation moved to here from ActionHandler
        ...
        try:
            ...
            if save_file.commit():
                ...
                editor.file_path = file_path # Update the editor's file path
                self.log_message("INFO", f"Saved to {file_path}")
                self._update_window_title() # Refresh main window title
                return True
            else:
    ...
    def _update_item_properties_from_move(self, moved_item):
        if self._current_edited_item_in_dock and self._current_edited_item_in_dock == moved_item:
             # Just update the existing editors rather than rebuilding the whole dock
    ...
    def closeEvent(self, event: QCloseEvent):
        """Overrides QMainWindow.closeEvent to check for unsaved changes in all tabs."""
        logger.info("MW_CLOSE: closeEvent received.")
        if self.py_sim_active: self.py_sim_ui_manager.on_stop_py_simulation(silent=True)
    ...
    # You can ignore 'region' if not needed
        if not hasattr(self, 'resource_estimation_dock') or not self.resource_estimation_dock.isVisible():
            return
    # ...existing code...
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str | None = None):
        ...
        for i, comment_data in enumerate(comments_data):
            ...
            cmd = AddItemCommand(self, comment_item, "Add Comment")
            self.undo_stack.push(cmd)
