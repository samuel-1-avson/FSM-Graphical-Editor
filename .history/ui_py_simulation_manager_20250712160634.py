...
class PySimulationUIManager(QObject):
    ...
    def on_toggle_real_time_simulation(self):
        ...
        if self.real_time_timer.isActive():
            self.real_time_timer.stop(); self.append_to_action_log(["Real-time simulation paused."])
        else:
            interval = self.py_sim_speed_slider.value(); self.real_time_timer.setInterval(interval); self.real_time_timer.start()
    ...
    def _update_internal_controls_enabled_state(self):
        ...
        # Start (Initialize) button state
        self.py_sim_start_btn.setEnabled(not sim_active)

        # Controls available when sim is active (initialized)
        self.py_sim_stop_btn.setEnabled(sim_active); self.py_sim_reset_btn.setEnabled(sim_active)
        run_controls_enabled = sim_active and not is_paused_at_bp; self.py_sim_run_pause_btn.setEnabled(run_controls_enabled); self.py_sim_speed_slider.setEnabled(run_controls_enabled)
        
        # Set Run/Pause button text and icon
        if is_running_real_time: self.py_sim_run_pause_btn.setText("Pause"); self.py_sim_run_pause_btn.setIcon(get_standard_icon(QStyle.SP_MediaPause, "Pause"))
        else: self.py_sim_run_pause_btn.setText("Run"); self.py_sim_run_pause_btn.setIcon(get_standard_icon(QStyle.SP_MediaPlay, "Run"))
    ...
    def _set_simulation_active_state(self, is_running: bool):
        if not is_running: self.real_time_timer.stop()
        super()._set_simulation_active_state(is_running)

    def append_to_action_log(self, log_entries: list[str]):
        if not self.py_sim_action_log_output: return
        time_color = QColor(COLOR_TEXT_SECONDARY).darker(110).name(); default_log_color = self.py_sim_action_log_output.palette().color(QPalette.Text).name(); 
        # --- FIX for AttributeError ---
        error_color = QColor(COLOR_ACCENT_ERROR).name(); warning_color = QColor(COLOR_ACCENT_WARNING).darker(10).name(); highlight_color = QColor(COLOR_ACCENT_PRIMARY).name(); 
        success_color = QColor(COLOR_ACCENT_SUCCESS).darker(110).name(); breakpoint_color = QColor(COLOR_ACCENT_WARNING).name()
        # --- END FIX ---
