...
class PySimulationUIManager(QObject):
        ...
        if state_name_to_highlight and editor.py_fsm_engine: 
            ...
            for item in editor.scene.items(): 
                if isinstance(item, GraphicsStateItem) and item.text_label == leaf_state:
                    item.set_py_sim_active_style(True); self._py_sim_currently_highlighted_item = item; item_to_highlight = item
                    if editor.view: editor.view.ensureVisible(item, 50, 50);
                    break
        
        # This check is moved from append_to_action_log to here for direct animation control.
        # It's triggered when the UI is updated *after* a step.
    ...
    def append_to_action_log(self, log_entries: list[str]):
        ...
        # Parse logs to find events that should trigger animations
    ...
    def on_start_py_simulation(self):
        ...
        if editor.py_sim_active: QMessageBox.information(self.mw, "Simulation Initialized", "Python simulation is already initialized."); return
