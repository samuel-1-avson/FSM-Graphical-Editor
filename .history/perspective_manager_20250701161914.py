...
class PerspectiveManager(QObject):
    ...
    def _apply_default_layout(self, name: str):
        ...
        self.mw.log_dock.setVisible(True) # Make log visible by default
        ...
        if name == self.mw.PERSPECTIVE_DESIGN_FOCUS:
            ...
            self.mw.addDockWidget(Qt.LeftDockWidgetArea, self.mw.minimap_dock)
            self.mw.tabifyDockWidget(self.mw.elements_palette_dock, self.mw.minimap_dock)
            ...
            self.mw.minimap_dock.setVisible(True)

            main_width = self.mw.width()
            self.mw.resizeDocks([self.mw.elements_palette_dock, self.mw.properties_dock], 
                                [int(main_width * 0.18), int(main_width * 0.22)], 
                                Qt.Horizontal)

            self.mw.elements_palette_dock.raise_()
    ...
    def reset_all(self):
        ...
        QMessageBox.information(self, "Perspectives Reset", "All layouts have been reset.")