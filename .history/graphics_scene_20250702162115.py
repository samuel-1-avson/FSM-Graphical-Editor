...
# --- FIX: Move local imports to top-level to avoid circular dependencies and follow best practices ---
...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None, custom_snippet_manager: CustomSnippetManager | None = None, settings_manager: SettingsManager | None = None):
        ...
        self._is_alt_dragging_transition = False # --- NEW: For drag-to-transition ---
        ...
        # Load colors from settings manager
    ...
    def drawBackground(self, painter: QPainter, rect: QRectF):
        ...
        # Update colors dynamically in case theme changed without scene recreation
        self.grid_pen_light.setColor(QColor(self.settings_manager.get("canvas_grid_minor_color")))
        self.grid_pen_dark.setColor(QColor(self.settings_manager.get("canvas_grid_major_color")))
        self._guideline_pen.setColor(QColor(self.settings_manager.get("canvas_snap_guideline_color")))
        self.setBackgroundBrush(QColor(config.COLOR_BACKGROUND_LIGHT))
    ...
    def set_mode(self, mode: str):
        ...
        # --- NEW: Reset alt-drag flag on mode change ---
    ...
    def _show_item_context_menu(self, item, event):
        menu = QMenu()
    ...
    def _handle_transition_click(self, clicked_state_item: GraphicsStateItem, click_pos: QPointF):
        ...
        if not self.transition_start_item:
            ...
            # --- FIX: Encapsulate cleanup and state reset ---
            def finalize_transition_drawing(success=True):
                if self.current_hovered_target_item:
                    self.current_hovered_target_item.set_potential_transition_target_style(False)
                    self.current_hovered_target_item = None
                if self._temp_transition_line:
                ...
                self.transition_start_item = None 
                self._is_alt_dragging_transition = False
                if success or self.current_mode == "transition":
                    self.set_mode("select")
            # --- END FIX ---
            ...
            initial_props = {
                ...
                'color': self.settings_manager.get("item_default_transition_color"),
                'description':"", 'control_offset_x':0, 'control_offset_y':0,
                'line_style_str': self.settings_manager.get("transition_default_line_style_str"), 
                'line_width': self.settings_manager.get("transition_default_line_width"),
                'arrowhead_style': self.settings_manager.get("transition_default_arrowhead_style"),
            ...
            if dialog.exec() == QDialog.Accepted:
                ...
                self.undo_stack.push(cmd) 
                self._log_to_parent("INFO", f"Added transition: {self.transition_start_item.text_label} -> {target_item.text_label} [{new_transition._compose_label_string()}]")
                finalize_transition_drawing(success=True)
            else:
                self._log_to_parent("INFO", "Transition addition cancelled by user.")
                finalize_transition_drawing(success=False)
...
class MinimapView(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self._viewport_rect_item.setPen(QPen(QColor(255, 0, 0, 150), 2))
        self._viewport_rect_item.setBrush(QBrush(QColor(255, 0, 0, 30)))
        
    def setMainView(self, view: ZoomableView | None):
        # Disconnect from old view if it exists
        if self.main_view:
            try:
                ...
                logger.debug("Error disconnecting signals from old main_view, likely already disconnected.")
    ...
    def update_viewport_rect(self):
        ...
        if not self.main_view or not self.scene():
            if self._viewport_rect_item.isVisible():
                self._viewport_rect_item.hide()
            return
        
        if not self._viewport_rect_item.isVisible():
    ...
    def setScene(self, scene: QGraphicsScene | None):
        ...
        # Disconnect from the old scene's changed signal
        if self.scene():
            try:
        ...
        if scene:
            ...
            self._viewport_rect_item.show()
            # Connect to the new scene's changed signal
            scene.changed.connect(self._on_scene_changed)
            # Perform an initial fit
            self._fit_contents()
            self.update_viewport_rect()
        else:
            if self._viewport_rect_item:
                self._viewport_rect_item.hide()

    def _on_scene_changed(self):
        """Slot to react to scene content changes."""
        # Use a single shot timer to coalesce multiple rapid changes (e.g., during a drag)
        if not hasattr(self, '_fit_timer'):
            self._fit_timer = QTimer(self)
            self._fit_timer.setSingleShot(True)
            self._fit_timer.setInterval(150) # Coalesce updates within 150ms
            self._fit_timer.timeout.connect(self._fit_contents)
        self._fit_timer.start()

    def _fit_contents(self):
        """Fits the view to the scene's content."""
        if self.scene():
            content_rect = self.scene().itemsBoundingRect()
