...
class PerspectiveManager(QObject):
    ...
    def _apply_default_layout(self, name: str):
        ...
        all_docks = [
            ...
            self.mw.resource_estimation_dock, self.mw.live_preview_dock
        ]
        ...
        # Common setup for all perspectives: tabify log and problems at the bottom
        ...
        # Configure docks based on the selected perspective
        if name == self.mw.PERSPECTIVE_DESIGN_FOCUS:
            ...
            # --- NEW: Set desired sizes for the side docks ---
            main_width = self.mw.width()
            # Assign a smaller, fixed-feeling percentage to the side docks
            left_dock_width = int(main_width * 0.15)  # 15% for Elements
            right_dock_width = int(main_width * 0.20) # 20% for Properties
            
            # Use resizeDocks to apply the new sizes, letting the center expand
            self.mw.resizeDocks([self.mw.elements_palette_dock], [left_dock_width], Qt.Horizontal)
            self.mw.resizeDocks([self.mw.properties_dock], [right_dock_width], Qt.Horizontal)
            ...
            main_width = self.mw.width()
            right_dock_width = int(main_width * 0.30)
            self.mw.resizeDocks([self.mw.py_sim_dock], [right_dock_width], Qt.Horizontal)
