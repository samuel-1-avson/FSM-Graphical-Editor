    ...
    def _update_properties_dock(self):
        ...
        if show_editor and hasattr(item_to_edit, 'get_data'):
            ...
            for prop_info in schema:
                ...
                # --- FIX START: Rename local variable `config` to `widget_config` ---
                if widget_config := prop_info.get('config'):
                    for method_name, args in widget_config.items(): getattr(editor_widget, method_name)(*args) if isinstance(args, tuple) else getattr(editor_widget, method_name)(args)
                # --- FIX END ---
