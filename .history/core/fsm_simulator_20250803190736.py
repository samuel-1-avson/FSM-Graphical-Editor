...
def check_code_safety_basic(code_str: str, known_vars: Set[str]) -> Tuple[bool, str]:
    ...
    disallowed_functions = [
        'open', 'eval', 'exec', 'exit', 'quit', 'input', 'compile', 
...
class FSMSimulator(QObject):
    ...
    def __init__(self, states_data: List[Dict[str, Any]], transitions_data: List[Dict[str, Any]], halt_on_action_error: bool = False):
        """
        Initializes the simulator.

        Args:
            states_data: A list of state dictionaries from the diagram data.
            transitions_data: A list of transition dictionaries.
            halt_on_action_error: If True, any error in user code will halt the simulation.
        """
        # --- NEW: Call the QObject parent constructor ---
        super().__init__()
        
        self.states: Dict[str, Dict[str, Any]] = {s['name']: s for s in states_data}
        self.transitions = transitions_data
        self._halt_on_error = halt_on_action_error
        
        # --- MODIFIED: Variables are now initialized based on a starting set ---
        ...
        self.current_state_path: List[Dict[str, Any]] = []
        self._internal_event_queue: List[str] = []
        
        # --- NEW: Attribute for formal simulation time ---
        self.stop_at_tick = -1  # -1 means run indefinitely
    ...
    def step(self, event_name: Optional[str] = None) -> Tuple[str, List[str]]:
        ...
        This involves running 'during' actions, processing the event queue,
        checking for transitions, and executing exit/entry actions.

        Args:
            event_name: An optional external event to add to the queue.

        Returns:
            A tuple of (current_state_name, list_of_log_messages).
        """
        # --- MODIFIED: Add check for stop_at_tick ---
