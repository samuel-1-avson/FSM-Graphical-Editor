...
class FSMSimulator(QObject):
    ...
    def reset(self) -> None:
        ...
        # --- MODIFICATION: Uses FsmModel IR methods ---
    ...
    def _enter_state(self, state: State) -> None:
        ...
        if state.entry_action:
            self._execute_action(state.entry_action.code)
    ...
    def step(self, event_name: Optional[str] = None) -> Tuple[str, List[str]]:
        ...
        # --- MODIFICATION: Logic now uses the IR model ---
        self.current_tick += 1
        self._action_log.clear()
        ...
        if current_leaf_state.during_action:
            self._execute_action(current_leaf_state.during_action.code)
            ...
            for i in range(len(self.current_state_path) - 1, -1, -1):
                ...
                for trans in self.model.transitions:
                    ...
                    if trans.condition and trans.condition.code:
                        try:
                            condition_ok = bool(eval(trans.condition.code, {"__builtins__": {}}, self._variables))
                        except Exception as e:
                            self.log_action(f"[EVAL ERROR] {e} in condition: '{trans.condition.code}'")
                            condition_ok = False
                    
                    if condition_ok:
                        transition_tuple = (trans.source_name, trans.target_name, trans.event)
                        if transition_tuple in self.breakpoints['transitions']:
                            # ... (breakpoint logic is unchanged)
                            return self.get_current_state_name(), self.get_last_executed_actions_log()
                        ...
                        for _ in range(len(self.current_state_path) - 1, i - 1, -1):
                            ...
                            if exiting_state.exit_action:
                                self._execute_action(exiting_state.exit_action.code)
                        
                        if trans.action:
                            self._execute_action(trans.action.code)
                        
                        new_state = self.model.states.get(trans.target_name)
                        if new_state:
