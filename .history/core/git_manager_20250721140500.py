...
class GitWorker(QObject):
    ...
    @pyqtSlot(list, str)
    def run_command(self, command: List[str], working_dir: str) -> None:
        """
...
class GitManager(QObject):
    ...
    def run_command_in_repo(self, command: List[str], file_path: str, finished_callback: Callable[[bool, str, str], None]) -> None:
        ...
        if self.worker and self.thread and self.thread.isRunning():
            # Disconnect previous temporary connections to avoid multiple callbacks
            try:
                self.worker.command_finished.disconnect()
            except TypeError:
                pass
            
            # Connect the one-shot callback for this specific command
