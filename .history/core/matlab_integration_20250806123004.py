...
class MatlabConnection(QObject):
    ...
    def _on_command_finished(self, success: bool, message: str, data: str, 
        ...
        if command_type == CommandType.MODEL_GENERATION and success:
            ...
            if model_name and slx_file_path and class_name:
                ...
                # --- NEW: Call setup_data_streaming after instantiation ---
                ...
                self._execute_command(setup_streaming_command)
                # --- END NEW ---
        ...
        if command_type == CommandType.SIMULATION:
            ...
            if success:
                ...
                self._model_cache[model_name] = {'path': metadata.get('output_path', ''), 'created': time.time(), 'metadata': metadata}
    ...
    def _create_enhanced_model_generation_script(self, fsm_model: FsmModel,
        ...
        """Create MATLAB script for model generation and the companion class file."""
        ...
        try:
            ...
            m_file_path = Path(output_path).with_suffix('.m')
            with open(m_file_path, 'w', encoding='utf-8') as f:
                f.write(class_content)
            logger.info(f"Generated MATLAB FSM class file at: {m_file_path}")
            m_file_generated = True
        except Exception as e:
            logger.error(f"Failed to write MATLAB class file: {e}", exc_info=True)
            m_file_generated = False
        
        slx_script_content = self._create_slx_generation_script(
    ...
    def _create_slx_generation_script(self, fsm_model: 'FsmModel', output_path: str, model_name: str, 
                                      options: Dict[str, bool]) -> str:
        """
        Creates the comprehensive MATLAB script for generating the .slx model file
        from the FSM Intermediate Representation (IR).
        """
        ...
        def generate_state_creation_code(states: List['State']) -> List[str]:
            ...
            # Use state properties for layout or calculate if missing
            has_positions = all('x' in state.properties and 'y' in state.properties for state in states)
            if not has_positions:
                script_lines.append("    % Calculating optimal state layout as positions were missing.")
            ...
            for i, state in enumerate(states):
                ...
                # Build the state label from IR Actions
                label_parts = [
                    f'entry: active_state_name = "{state.name}"; active_state_index = {i+1};'
                ]
                if state.entry_action and state.entry_action.code:
                    escaped_code = state.entry_action.code.replace("'", "''").replace('\n', '; ')
                    label_parts.append(f"entry: try; {escaped_code}; catch; end;")
                if state.during_action and state.during_action.code:
                    escaped_code = state.during_action.code.replace("'", "''").replace('\n', '; ')
                    label_parts.append(f"during: try; {escaped_code}; catch; end;")
                if state.exit_action and state.exit_action.code:
                    escaped_code = state.exit_action.code.replace("'", "''").replace('\n', '; ')
                    label_parts.append(f"exit: try; {escaped_code}; catch; end;")
                
                state_label = "\\n".join(label_parts)
                
                script_lines.extend([
                    f"    % Create state: {state.name}",
                    f"    {state_id} = Stateflow.State(chartSFObj);",
                    f"    {state_id}.Name = '{state.name.replace("'", "''")}';",
                    f"    {state_id}.Position = [{sf_x:.1f}, {sf_y:.1f}, {sf_w:.1f}, {sf_h:.1f}];",
                    f"    {state_id}.LabelString = '{state_label}';",
                    f"    {state_id}.Description = '{state.description.replace("'", "''")}';",
                    f"    stateHandles('{state.name.replace("'", "''")}') = {state_id};",
                    f"    stateIndexMap('{state.name.replace("'", "''")}') = {i+1};"
                ])

                if state.is_initial:
                    script_lines.extend([
                        f"    % Set {state.name} as initial state",
                        ...
                        f"    defaultTrans_{i}.Description = 'Initial transition to {state.name}';"
                    ])
        ...
        def generate_transition_creation_code(transitions: List['Transition']) -> List[str]:
            ...
            for i, trans in enumerate(transitions):
                src_name = trans.source_name.replace("'", "''")
                dst_name = trans.target_name.replace("'", "''")
                
                label_parts = []
                if trans.event:
                    label_parts.append(trans.event)
                if trans.condition and trans.condition.code:
                    label_parts.append(f"[{trans.condition.code}]")
                
                action_parts = []
                if trans.action and trans.action.code:
                    escaped_action = trans.action.code.replace("'", "''").replace('\n', '; ')
                ...
                script_lines.extend([
                    ...
                    f"        trans_{i}.LabelString = '{trans_label}';",
                    "    else",
        ...
        script_lines = [
            ...
            "    open_system(hModel);",
            # ... model configuration (Solver, StopTime, etc.) ...
            ...
            "    activeStateData.Name = 'active_state_name'; activeStateData.Scope = 'Output'; activeStateData.DataType = 'string';",
            "    stateIndexData = Stateflow.Data(chartSFObj);",
            "    stateIndexData.Name = 'active_state_index'; stateIndexData.Scope = 'Output'; stateIndexData.DataType = 'uint8';",
            "    transitionCountData = Stateflow.Data(chartSFObj);",
            "    transitionCountData.Name = 'transition_count'; transitionCountData.Scope = 'Output'; transitionCountData.DataType = 'uint32';",
            "    transCountLocal = Stateflow.Data(chartSFObj);",
            "    transCountLocal.Name = 'trans_count_local'; transCountLocal.Scope = 'Local'; transCountLocal.DataType = 'uint32';",
            "    fprintf('FSM_PROGRESS: Adding output blocks...\\n');",
            "    add_block('simulink/Sinks/To Workspace', [modelNameVar '/State_Name_Out'], 'VariableName', 'state_names', 'SaveFormat', 'Timeseries');",
            "    add_block('simulink/Sinks/To Workspace', [modelNameVar '/State_Index_Out'], 'VariableName', 'state_indices', 'SaveFormat', 'Timeseries');",
            "    add_block('simulink/Sinks/To Workspace', [modelNameVar '/Transition_Count_Out'], 'VariableName', 'transition_counts', 'SaveFormat', 'Timeseries');",
        ...
        script_lines.extend([
            ...
            "    if optimizeLayout, Simulink.BlockDiagram.arrangeSystem(modelNameVar); end",
    ...
    def run_simulation(self, model_path: str, config: SimulationConfig = None, 
        ...
        script_content = """
        try
            % Call the run method on the persistent FSM instance
            resultsJson = FSM_INSTANCE.run(py_sim_config);
            fprintf('MATLAB_SCRIPT_SUCCESS:%s\\n', resultsJson);
        catch e
            % Enhanced error reporting
            errorReport = struct('message', e.message, 'identifier', e.identifier);
