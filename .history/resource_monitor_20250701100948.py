...
class ResourceMonitorWorker(QObject):
    ...
    def __init__(self, interval_ms=2000, parent=None):
        ...
        self._transient_error_count = 0 # To throttle logging
                ...
                try:
                    ...
                    if self._nvml_initialized and self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                        try:
                            ...
                            self._transient_error_count = 0 # Reset error count on success
                        except pynvml.NVMLError as e:
                            # --- KEY CHANGE: Gracefully handle the error without re-initializing ---
                            self._transient_error_count += 1
                            # Log the warning less frequently to avoid spamming the console
                            if self._transient_error_count % 15 == 1: # Log on 1st, 16th, 31st... failure
...
class ResourceMonitorManager(QObject):
    ...
    def setup_and_start_monitor(self):
        ...
        # --- KEY CHANGE: Ensure worker cleans up its own NVML session upon finishing ---
    ...
    def stop_monitoring_system(self):
        ...
        # --- KEY CHANGE: Cleanup references after stopping ---
        self.worker = None
        self.thread = None
