# fsm_designer_project/services/matlab_integration.py
import sys
import os
import io
import logging
...
class EngineHealthMonitor:
    ...
    def _trigger_recovery(self):
        ...
        try:
            # --- FIX: Use invokeMethod to call the slot from the non-Qt thread ---
            QMetaObject.invokeMethod(self.worker, "_handle_engine_failure", Qt.ConnectionType.QueuedConnection,
                                     Q_ARG(str, "Health check failures"))
        except Exception:
...
class MatlabEngineWorker(QObject):
    ...
    def initialize_engine(self):
        ...
        if self.engine:
            self.engine_status_changed.emit(EngineState.CONNECTED, "Engine already initialized")
            return
        ...
        try:
            ...
            if self._auto_recovery_count < self._max_auto_recovery_attempts:
                # --- FIX: Parent the timer to self to ensure it's in the correct thread ---
    ...
    def execute_command(self, command: MatlabCommand):
        ...
        if self.state == EngineState.DISCONNECTED:
            ...
            self._emit_command_result(False, "MATLAB Engine disconnected.", "", 
    ...
    # --- FIX: Make this a slot to be called from a different thread ---
    @pyqtSlot(str)
    ...
    # --- FIX: Make this a slot to be callable by QTimer ---
    @pyqtSlot()
    def _attempt_auto_recovery(self):
        ...
        # --- FIX: Correct method name and call directly (already in correct thread) ---
...
class MatlabConnection(QObject):
    ...
    def _setup_worker_thread(self):
        ...
        # --- FIX: Connect to the correct method name ---
    ...
    def _create_enhanced_model_generation_script(self, fsm_model: FsmModel,
        ...
        # This calculates the path relative to the current file's location
