...
class LogLevel(Enum):
    ...
    DEBUG = (logging.DEBUG, theme_config.COLOR_TEXT_EDITOR_DARK_SECONDARY, "🔍")
    INFO = (logging.INFO, theme_config.COLOR_TEXT_EDITOR_DARK_PRIMARY, "ℹ️")
    WARNING = (logging.WARNING, theme_config.COLOR_ACCENT_WARNING, "⚠️")
    ERROR = (logging.ERROR, theme_config.COLOR_ACCENT_ERROR, "❌")
    CRITICAL = (logging.CRITICAL, theme_config.COLOR_ACCENT_ERROR, "🔥")
...
class LogTheme:
    ...
    def create_dark_theme(cls):
        theme = cls("Dark")
        theme.colors = {
            'background': theme_config.COLOR_BACKGROUND_EDITOR_DARK,
            'text_primary': theme_config.COLOR_TEXT_EDITOR_DARK_PRIMARY,
            'text_secondary': theme_config.COLOR_TEXT_EDITOR_DARK_SECONDARY,
            'accent': theme_config.COLOR_ACCENT_PRIMARY,
            'success': theme_config.COLOR_ACCENT_SUCCESS,
            'warning': theme_config.COLOR_ACCENT_WARNING,
            'error': theme_config.COLOR_ACCENT_ERROR,
            'border': '#3c3c3c',
            'selection': '#404040'
        }
        theme.fonts = {
...
class AdvancedHtmlFormatter(logging.Formatter):
    ...
    def _format_html(self, entry: LogEntry) -> str:
        ...
        # Exception info
        if entry.exc_info:
            exc_html = f"<pre style='color:{self.theme.colors['error']}; margin:2px 0; padding:4px; background-color:#2d1b1b; border-left:3px solid {self.theme.colors['error']};'>{html.escape(entry.exc_info)}</pre>"
