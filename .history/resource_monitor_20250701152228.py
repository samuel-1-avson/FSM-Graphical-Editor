...
class ResourceMonitorWorker(QObject):
    ...
    def _monitor_resources(self):
        ...
        # --- FIX: The loop should only be controlled by the stop flag ---
...
class ResourceMonitorManager(QObject):
    ...
    def stop_monitoring_system(self):
        ...
        if self.thread and self.thread.isRunning() and self.worker:
            ...
            # Signal the worker to stop its loop. The worker will emit finished_signal when its loop exits.
            QMetaObject.invokeMethod(self.worker, "stop_monitoring", Qt.QueuedConnection)
            
            # Wait for the thread to process the stop signal and finish.
            if not self.thread.wait(1000): # Give it 1 second to stop.
