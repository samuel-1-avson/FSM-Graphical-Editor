...
def generate_python_fsm_file(diagram_data: dict, output_dir: str, class_name_base: str) -> str:
    ...
    py_code = template.render(context)
    
    file_name = f"{sanitize_python_identifier(class_name.lower())}.py"
    ...
    try:
        ...
        logger.error(f"IOError writing Python FSM file: {e}", exc_info=True)
        raise