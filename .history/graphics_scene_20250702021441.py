...
# --- FIX: REMOVE THE GLOBAL IMPORT that could contribute to the cycle ---
...
class DiagramScene(QGraphicsScene):
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            # --- NEW: Alt+Drag to create transition ---
    ...
    def edit_item_properties(self, item):
        # --- FIX: LOCAL IMPORTS TO PREVENT ALL CIRCULAR DEPENDENCIES ---
        ...
        # Pass the ai_manager instance to dialog constructors
        ...
        if DialogType == StatePropertiesDialog:
            ...
            # --- FIX: Pass the settings_manager ---
            dialog = TransitionPropertiesDialog(self.settings_manager, self.custom_snippet_manager, ai_manager, parent=dialog_parent, current_properties=old_props)
        else: 
    ...
    def _handle_transition_click(self, clicked_state_item: GraphicsStateItem, click_pos: QPointF):
        ...
        if not self.transition_start_item:
            ...
            ai_manager = self.parent_window.ai_chat_ui_manager if hasattr(self.parent_window, 'ai_chat_ui_manager') and isinstance(self.parent_window.ai_chat_ui_manager, AIChatUIManager) else None
            # --- FIX: Pass settings_manager ---
            dialog = TransitionPropertiesDialog(self.settings_manager, self.custom_snippet_manager, ai_manager, parent=dialog_parent, current_properties=initial_props, is_new_transition=True)
...
class MinimapView(QGraphicsView):
    ...
    def setMainView(self, view: ZoomableView | None):
        self.main_view = view
        if self.scene() and self._viewport_rect_item.scene() != self.scene():
            self.scene().addItem(self._viewport_rect_item)
        ...
        if self.main_view:
            ...
            # A timer-based update on resize is more robust
    ...
    def setScene(self, scene: QGraphicsScene | None):
        ...
        if scene and self._viewport_rect_item.scene() != scene:
