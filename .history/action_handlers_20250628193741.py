...
class ActionHandler(QObject):
    ...
    def on_toggle_snap_to_grid(self, checked):
        # This action now applies to all open editors' scenes
        for i in range(self.mw.tab_widget.count()):
            editor = self.mw.tab_widget.widget(i)
            if editor:
                editor.scene.snap_to_grid_enabled = checked
        logger.info(f"Snap to Grid {'enabled' if checked else 'disabled'} for all editors.")

    @pyqtSlot(bool)
    def on_toggle_snap_to_objects(self, checked):
        # Applies to all open editors
        for i in range(self.mw.tab_widget.count()):
            editor = self.mw.tab_widget.widget(i)
            if editor:
                editor.scene.snap_to_objects_enabled = checked
        logger.info(f"Snap to Objects {'enabled' if checked else 'disabled'} for all editors.")
    ...
    def on_toggle_show_snap_guidelines(self, checked):
        ...
        for i in range(self.mw.tab_widget.count()):
            editor = self.mw.tab_widget.widget(i)
            if hasattr(editor.scene, '_show_dynamic_snap_guidelines'):
                editor.scene._show_dynamic_snap_guidelines = checked
                if not checked: editor.scene._clear_dynamic_guidelines()
        logger.info(f"Dynamic Snap Guidelines {'shown' if checked else 'hidden'} for all editors.")
