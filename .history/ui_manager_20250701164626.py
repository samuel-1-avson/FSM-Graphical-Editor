...
class UIManager(QObject): 
    ...
    def update_undo_redo_state(self):
        """
        Updates the enabled/disabled state and tooltips of the Undo and Redo actions
        based on the current editor's undo stack.
        """
        mw = self.mw
        editor = mw.current_editor() if hasattr(mw, "current_editor") else None
        can_undo = editor and hasattr(editor, "undo_stack") and editor.undo_stack.canUndo()
        can_redo = editor and hasattr(editor, "undo_stack") and editor.undo_stack.canRedo()

        if hasattr(mw, "undo_action"):
            mw.undo_action.setEnabled(bool(can_undo))
        ...
        if hasattr(mw, "redo_action"):
            mw.redo_action.setEnabled(bool(can_redo))
