...
from PyQt5.QtWidgets import (
    ...
    QToolButton, QGroupBox, QComboBox, QProgressBar, QFormLayout,QObject
)
...
class UIManager(QObject): 
    ...
    def setup_ui(self):
        ...
        # self.mw.setGeometry(50, 50, 1600, 1000) # Geometry is now restored in main.py
    ...
    def _create_actions(self):
        ...
        # 'Open Recent' submenu is created directly in _create_menus
    ...
    def _create_menus(self):
        ...
        self.recent_files_menu.aboutToShow.connect(mw._populate_recent_files_menu)
    ...
    def _create_docks(self):
        ...
        docks_to_create = ["ElementsPaletteDock", "PropertiesDock", "LogDock", "ProblemsDock", "PySimDock", "AIChatbotDock", "IDEDock", "ResourceEstimationDock"]
        for name in docks_to_create: setattr(mw, name.lower(), QDockWidget(name.replace("Dock", "").replace("_", " "), mw, objectName=name))
        self._populate_elements_palette_dock(); self._populate_properties_dock()
        ...
        # Add toggle actions to View->Docks menu
        mw.docks_menu.clear(); dock_list = [getattr(mw, name.lower()) for name in docks_to_create]
    ...
    def _create_status_bar(self):
        ...
        def add_seg(text, tip, name, min_width=60):
            lbl = QLabel(text); lbl.setToolTip(tip); lbl.setObjectName(name); lbl.setMinimumWidth(min_width)
            status_bar.addPermanentWidget(lbl); return lbl
        
        mw.main_op_status_label = QLabel("Ready"); status_bar.addWidget(mw.main_op_status_label, 1)
        add_seg("Mode: Select", "Interaction Mode", "InteractionModeStatusLabel"); add_seg("IDE: Idle", "IDE Script", "IdeFileStatusLabel")
        add_seg("100%", "Zoom", "ZoomStatusLabel")
        # Resource monitor widgets created by its manager
        mw.py_sim_status_label = add_seg("PySim: Idle", "Python Sim Status", "PySimStatusLabel")
        mw.matlab_status_label = add_seg("MATLAB: Init...", "MATLAB Status", "MatlabStatusLabel")
        mw.internet_status_label = add_seg("Net: Init...", "Internet", "InternetStatusLabel")
