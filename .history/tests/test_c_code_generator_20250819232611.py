...
import pytest
from fsm_designer_project.codegen.c_code_generator import sanitize_c_identifier, generate_c_code_content
...
def test_sanitize_c_identifier():
    ...
    assert sanitize_c_identifier("some_var(copy)") == "some_var_copy_"
    assert sanitize_c_identifier("") == "s_Unnamed"
    assert sanitize_c_identifier("____") == "____"
...
def test_generate_c_code_from_simple_fsm():
    ...
    code = generate_c_code_content(diagram_data, "my_blinker", "Generic C (Header/Source Pair)")
    ...
    assert "void on_entry_Off(void);" in code['h']
    assert "void on_trans_Off_to_On(void);" in code['h']
    assert "void on_exit_On(void);" in code['h']

    # Test Source File (.c)
    assert '#include "my_blinker.h"' in code['c']
    assert "static FSM_StateId_t current_fsm_state;" in code['c']
    ...
    assert "void on_entry_Off(void)" in code['c']
    assert "Original code (commented):" in code['c']
