...
from .services import (
    GitManager, HardwareLinkManager,
    EngineState, CommandType, ResourceMonitorManager
)
from .managers import (
    UIManager, IDEManager, ActionHandler,
    PerspectiveManager, DataDictionaryManager, MatlabSimulationManager,
    CSimulationManager, signal_bus
)
from .codegen import (
    generate_python_fsm_code, generate_c_code_content,
    generate_plantuml_text, generate_mermaid_text
)
...
from .managers.project_manager import PROJECT_FILE_EXTENSION, ProjectManager
from .services.file_loader import FileLoaderWorker
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, context: ApplicationContext):
        ...
        self.matlab_connection = self.context.matlab_connection
        ...
        self.animation_manager = AnimationManager(parent=self)
        ...
        self.file_loader_thread.setObjectName("FileLoaderThread")
        self.file_loader_worker = FileLoaderWorker()
        self.file_loader_worker.moveToThread(self.file_loader_thread)
        self.file_loader_worker.load_finished.connect(self._on_file_load_finished)
        self.file_loader_thread.start()
        self._target_editor_for_load: EditorWidget | None = None
        self._target_path_for_load: str | None = None
        ...
        QTimer.singleShot(100, self.perspective_manager.apply_perspective(self.perspective_manager.current_perspective_name))
        QTimer.singleShot(150, self._restore_session) # Restore session after UI is built
    ...
    def _connect_application_signals(self):
        ...
        self.welcome_widget.showGuideRequested.connect(self.action_handler.help_handler.on_show_quick_start)
        self.welcome_widget.showExamplesRequested.connect(lambda: self.action_handler.file_handler._open_example_file("traffic_light.bsm"))

        self.properties_edit_dialog_button.clicked.connect(self.ui_manager.on_advanced_edit_button_clicked)
        ...
        self.project_tree_view.customContextMenuRequested.connect(self.action_handler.file_handler.on_project_explorer_context_menu)

        self.perspective_manager.populate_menu()
        self.save_perspective_action.triggered.connect(self.perspective_manager.save_current_as)
        self.reset_perspectives_action.triggered.connect(self.perspective_manager.reset_all)
        ...
        if self.py_sim_ui_manager:
            ...
            self.ai_chat_ui_manager.applyFixRequested.connect(self.action_handler.edit_handler.apply_ai_fix)
        ...
        if self.ide_manager:
            ...
            self.ide_manager.ide_file_path_changed.connect(self.ui_manager.update_window_title)
        ...
        signal_bus.project_loaded.connect(self.on_project_loaded)
        signal_bus.project_closed.connect(self.on_project_closed)
        signal_bus.status_message_posted.connect(self.log_message)
        signal_bus.ui_element_focus_requested.connect(self.focus_on_item)
    ...
    def _populate_recent_files_menu(self):
        ...
        for i, file_path in enumerate(recent_files):
            ...
            action.triggered.connect(self.action_handler.file_handler.on_open_recent_file)
    ...
    def _apply_theme(self, theme_name: str):
        ...
        for i in range(self.tab_widget.count()):
            ...
            if editor:
                editor.scene.setBackgroundBrush(QColor(theme_config.COLOR_BACKGROUND_LIGHT))
                editor.scene.grid_pen_light = QPen(QColor(theme_config.COLOR_GRID_MINOR), 0.7, Qt.PenStyle.DotLine)
                editor.scene.grid_pen_dark = QPen(QColor(theme_config.COLOR_GRID_MAJOR), 0.9, Qt.PenStyle.SolidLine)
                editor.scene.update() 
    ...
    def _create_and_load_new_tab(self, file_path: str):
        ...
        self._load_into_editor(new_editor, file_path)
    ...
    def _connect_editor_signals(self, editor: EditorWidget):
        ...
        editor.scene.modifiedStatusChanged.connect(self.ui_manager.update_window_title) 
        ...
        editor.scene.item_edit_requested.connect(self.on_item_edit_requested)
    ...
    def add_new_editor_tab(self) -> EditorWidget:
        new_editor = EditorWidget(self, self.asset_manager)
        new_editor.scene.settings_manager = self.settings_manager
        new_editor.scene.asset_manager = self.asset_manager
        ...
        if hasattr(self, 'live_preview_editor'):
            self.live_preview_editor.setStyleSheet(f"""
                QPlainTextEdit#LivePreviewEditor {{
                    border: 1px solid {theme_config.COLOR_ACCENT_WARNING};
                    background-color: {QColor(theme_config.COLOR_ACCENT_WARNING).lighter(185).name()};
                }}
            """)
    ...
    def on_project_loaded(self, project_path: str, project_data: dict):
        ...
        self.action_handler.file_handler.add_to_recent_files(project_path)
    ...
    def on_item_edit_requested(self, item: QGraphicsItem):
        ...
        if DialogType == StatePropertiesDialog:
            dialog = DialogType(self.settings_manager, self.asset_manager, parent=self, current_properties=old_props, is_new_state=False, scene_ref=editor.scene)
        elif DialogType == TransitionPropertiesDialog:
            dialog = TransitionPropertiesDialog(self.asset_manager, parent=self, current_properties=old_props)
        else: 
    ...
    def _on_raw_data_received(self, data: str):
        if hasattr(self, 'serial_monitor_output'):
            ...
            formatted_line = (
                f"<span style='color: {theme_config.COLOR_ACCENT_PRIMARY};'><b>RX <<</b></span> "
                f"<span style='color: {theme_config.COLOR_TEXT_SECONDARY};'>[{timestamp}]</span> "
    ...
    def _on_raw_data_sent(self, data: str):
        if hasattr(self, 'serial_monitor_output'):
            timestamp = QTime.currentTime().toString('hh:mm:ss.zzz')
            formatted_line = (
                f"<span style='color: {theme_config.COLOR_ACCENT_SECONDARY};'><b>TX >></b></span> "
                f"<span style='color: {theme_config.COLOR_TEXT_SECONDARY};'>[{timestamp}]</span> "
                f"<span>{html.escape(data)}</span>"
            )
            self.serial_monitor_output.append(formatted_line)
    ...
    def _on_open_recent_from_welcome(self, file_path: str):
        ...
        if file_path and os.path.exists(file_path):
            ...
            if hasattr(self, 'action_handler'):
                self.action_handler.file_handler.remove_from_recent_files(file_path)
    ...
    def _update_live_preview(self):
        ...
        self.live_preview_editor.setStyleSheet(f"""
                ...
                background-color: {theme_config.COLOR_BACKGROUND_EDITOR_DARK};
                color: {theme_config.COLOR_TEXT_EDITOR_DARK_PRIMARY};
                border: 1px solid {theme_config.COLOR_BORDER_DARK};
                border-radius: 3px;
                padding: 6px;
                selection-background-color: {QColor(theme_config.COLOR_ACCENT_PRIMARY).darker(110).name()};
                selection-color: {theme_config.COLOR_TEXT_ON_ACCENT};
                }}
        """)
    ...
    def _prompt_save_on_close(self, editor: EditorWidget) -> bool:
        ...
        if reply == QMessageBox.StandardButton.Save: return self.action_handler.file_handler.on_save_file() 
        return reply != QMessageBox.StandardButton.Cancel
    
    
    def _load_into_editor(self, editor: EditorWidget, file_path: str):
        self.log_message("INFO", f"Starting asynchronous load of '{os.path.basename(file_path)}'...")
        if hasattr(self, 'progress_bar'): self.progress_bar.setVisible(True)
        self.setEnabled(False)
        
        self._target_editor_for_load = editor
        self._target_path_for_load = file_path
        
        schema = BSM_SCHEMA
        if schema.get("title") == "Schema Loading Failed":
            msg = "Internal Error: Could not load diagram validation schema."
            self.log_message("ERROR", msg)
            self._on_file_load_finished({}, msg)
            return

        QMetaObject.invokeMethod(
            self.file_loader_worker, "load_file", Qt.ConnectionType.QueuedConnection,
            Q_ARG(str, file_path), Q_ARG(dict, schema)
        )
    
    
    
    # --- NEW slot to handle the result from the worker ---
    @pyqtSlot(dict, str)
    def _on_file_load_finished(self, diagram_data: dict, error_string: str):
        if hasattr(self, 'progress_bar'): self.progress_bar.setVisible(False)
        self.setEnabled(True)
        
        editor = self._target_editor_for_load
        file_path = self._target_path_for_load

        if not editor or not file_path:
            logger.error("File load finished but target editor/path was lost.")
            return

        if error_string:
            QMessageBox.critical(self, "Error Opening File", error_string)
            self.log_message("ERROR", f"Failed to load '{os.path.basename(file_path)}': {error_string}")
            index = self.tab_widget.indexOf(editor)
            if index != -1:
                self.tab_widget.removeTab(index)
            editor.deleteLater()
        else:
            editor.scene.load_diagram_data(diagram_data)
            editor.file_path = file_path
            editor.set_dirty(False)
            editor.undo_stack.clear()
            self.action_handler.file_handler.add_to_recent_files(file_path)
            
            index = self.tab_widget.indexOf(editor)
            if index != -1:
                self.tab_widget.setTabText(index, editor.get_tab_title())
            
            self.ui_manager.update_window_title()
            
            if self.git_manager:
                QTimer.singleShot(50, lambda p=file_path: self.git_manager.check_file_status(p))
            
            self.log_message("INFO", f"Finished loading '{os.path.basename(file_path)}'.")

        self._target_editor_for_load = None
        self._target_path_for_load = None
    ...
    # --- THIS IS THE UX ENHANCEMENT ---
    # Pan the view to ensure the selected item is visible and centered.
    item_rect = item_to_focus.sceneBoundingRect()
    padding = 50 # Add some padding around the item
    view_rect_with_padding = item_rect.adjusted(-padding, -padding, padding, padding)
    if editor.view:
        editor.view.fitInView(view_rect_with_padding, Qt.AspectRatioMode.KeepAspectRatio)
    # --- END ENHANCEMENT ---
    ...
    def _restore_session(self):
        """Restores open files from the previous session."""
        open_files = self.settings_manager.get("session_open_files", [])
        current_index = self.settings_manager.get("session_current_index", 0)
        
        if open_files:
            self.log_message("INFO", "Restoring previous session...")
            for file_path in open_files:
                if os.path.exists(file_path):
                    self._create_and_load_new_tab(file_path)
                else:
                    self.log_message("WARNING", f"Could not restore session tab: File not found at '{file_path}'")
                    # Clean up non-existent files from recent list as well
                    self.action_handler.file_handler.remove_from_recent_files(file_path)
    ...
    def closeEvent(self, event: QCloseEvent):
        """Overrides QMainWindow.closeEvent to save session and check for unsaved changes."""
        ...
        # This now handles saving all open diagram tabs
        if not self.action_handler.file_handler.on_close_project():
            event.ignore(); return
 
        # --- NEW: Save session state ---
        open_files = []
        for i in range(self.tab_widget.count()):
            editor = self.tab_widget.widget(i)
        ...
        self.settings_manager.set("session_open_files", open_files)
        self.settings_manager.set("session_current_index", self.tab_widget.currentIndex())
        # --- END NEW ---
        ...
        self.matlab_sim_manager.shutdown()
        ...
        logger.info("Session state saved. Application closeEvent accepted.")
        event.accept()
    ...
    def on_ask_ai_about_validation_issue(self):
        ...
        # --- MODIFIED: Get diagram context and build a better prompt ---
        editor = self.current_editor()
        diagram_context = ""
        if editor:
            diagram_data = editor.scene.get_diagram_data()
            # Use a lean version for the prompt to save tokens and focus the AI
            lean_diagram_data = {
            ...
            diagram_context = json.dumps(lean_diagram_data)

        prompt = f"""I am designing a Finite State Machine. My application's validation tool reported the following error or warning:

**Error/Warning:** "{issue_text}"

Here is the full JSON of my current diagram for context:
```json
{diagram_context}
```
Please explain what this means in the context of my specific FSM and propose a specific, one-step fix in a `[BSM_FIX]` JSON block to resolve this single issue. Be concise."""
    ...
    def _update_window_title(self):
        ...
        title_parts.append(f"- {config.APP_NAME}{pysim_suffix}")
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str | None = None):
        ...
        self.action_handler.view_handler.on_fit_diagram_in_view() 
...
def main_entry_point():
    ...
    # 1. Create the application's core context FIRST.
    app_context = ApplicationContext()

    # 2. Make the settings manager available globally via the QApplication instance.
    #    This is a useful pattern to avoid passing the context to every single dialog.
    if not hasattr(QApplication.instance(), 'settings_manager'):
        QApplication.instance().settings_manager = app_context.settings_manager

    # 3. Pass the context to the MainWindow upon creation.
    main_win = MainWindow(context=app_context)
    main_win.show()
    sys.exit(app.exec())
