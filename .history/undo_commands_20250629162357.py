...
class AddItemCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        if self.item_type == GraphicsStateItem.Type:
            ...
            comment_data = self.item_data.copy()
            comment_data.pop('width', None)  # Remove width if present
            item_to_add = GraphicsCommentItem(**comment_data)
            ...
            if start_node and end_node:
                ...
                if self.item_type == "transition":
                    # Map props to only the valid constructor arguments
                    event_str = props.get("event", "")
                    condition_str = props.get("condition", "")
                    action_language = props.get("action_language", config.DEFAULT_EXECUTION_ENV)
                    action_str = props.get("action", "")
                    color = props.get("properties", {}).get("color", config.COLOR_ITEM_TRANSITION_DEFAULT)
                    description = props.get("description", "")

                    item_to_add = GraphicsTransitionItem(
                        start_node,
                        end_node,
                        event_str=event_str,
                ...
                item_to_add = None  # Explicitly set to None to avoid further errors
