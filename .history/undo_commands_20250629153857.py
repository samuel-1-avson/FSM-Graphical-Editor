...
# CORRECTED: Only use relative imports for files within the package.
...
class RemoveItemsCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        for item_data in self.removed_items_data:
            ...
            if item_type == GraphicsStateItem.Type:
                ...
                for scene_item in self.scene.items():
                    if isinstance(scene_item, GraphicsCommentItem) and \
                       scene_item.toPlainText() == item_data['text'] and \
                       (scene_item.pos() - QPointF(item_data['x'], item_data['y'])).manhattanLength() < 0.1:
                        item_to_remove = scene_item
                        break
            elif item_type == GraphicsTransitionItem.Type:
                 for scene_item in self.scene.items():
                     if isinstance(scene_item, GraphicsTransitionItem) and \
                        scene_item.start_item and scene_item.start_item.text_label == item_data['_start_name'] and \
            ...
            if item_to_remove and item_to_remove.scene() == self.scene:
                ...
                logger.debug(f"RemoveItemsCommand: Redo - Removed item type {item_type} with data {item_data.get('name', item_data.get('text', 'Transition'))}")

        self.scene.set_dirty(True)
        self.scene.scene_content_changed_for_find.emit()
        self.scene.run_all_validations(f"RemoveItemsCommand_Redo_{len(items_actually_removed_this_redo)}")
    ...
    def undo(self):
        ...
        for item_data in self.removed_items_data:
            ...
            logger.debug(f"RemoveItemsCommand: Undo - Attempting to recreate item {item_name_for_log} of type {item_type}")

            if item_type == GraphicsStateItem.Type:
                if self.scene.get_state_by_name(item_data['name']):
                    logger.warning(f"RemoveItemsCommand: Undo - State '{item_data['name']}' already exists. Skipping recreation.")
                    continue
                state = GraphicsStateItem(**item_data) # Use dictionary unpacking for simplicity
                ...
                comment = GraphicsCommentItem(
                    item_data['x'], item_data['y'], item_data['text'],
                    item_data.get('font_family'), item_data.get('font_size'), item_data.get('font_italic')
                )
        ...
        for item_data in self.removed_items_data:
            if item_data.get('_type') == GraphicsTransitionItem.Type:
                src_item = states_map_for_undo.get(item_data['_start_name']) or self.scene.get_state_by_name(item_data['_start_name'])
                tgt_item = states_map_for_undo.get(item_data['_end_name']) or self.scene.get_state_by_name(item_data['_end_name'])

                if src_item and tgt_item:
                    trans_props = {k: v for k, v in item_data.items() if not k.startswith('_')}
                    trans = GraphicsTransitionItem(src_item, tgt_item, **trans_props)
                    self.scene.addItem(trans)
                    newly_re_added_instances.append(trans)
                    logger.debug(f"RemoveItemsCommand: Undo - Re-added transition '{item_data.get('event', 'Unnamed')}'")
                else:
                    log_msg = f"Error (Undo Remove): Could not re-link transition. States missing for '{item_data.get('event','Unnamed')}'"
        ...
        self.scene.set_dirty(True)
        self.scene.scene_content_changed_for_find.emit()
        self.scene.run_all_validations(f"RemoveItemsCommand_Undo_{len(newly_re_added_instances)}")
...
class MoveItemsCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        if self.scene_ref: self.scene_ref.run_all_validations("MoveItemsCommand_Redo")
    ...
    def undo(self):
        ...
        if self.scene_ref: self.scene_ref.run_all_validations("MoveItemsCommand_Undo")
...
class EditItemPropertiesCommand(QUndoCommand):
    def __init__(self, item, old_props_data, new_props_data, description="Edit Properties"):
        ...
        logger.debug(f"EditItemPropertiesCommand: Initialized for item '{item_name_for_log}'.")
    ...
    def _apply_properties(self, props_to_apply):
        ...
        if isinstance(self.item, GraphicsStateItem):
            ...
            # Use dictionary unpacking for a cleaner call
            ...
            # Transition properties require special handling for offset
    ...
    def redo(self):
        logger.debug("EditItemPropertiesCommand: Redo - Applying new properties to item.")
        self._apply_properties(self.new_props_data)
        if self.scene_ref: self.scene_ref.run_all_validations("EditItemProperties_Redo")

    def undo(self):
        logger.debug("EditItemPropertiesCommand: Undo - Applying old properties to item.")
        self._apply_properties(self.old_props_data)
        if self.scene_ref: self.scene_ref.run_all_validations("EditItemProperties_Undo")