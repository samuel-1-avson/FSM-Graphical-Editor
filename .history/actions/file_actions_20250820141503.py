...
# --- MODIFIED: Import the new helper ---
from ..utils.helpers import _get_bundled_file_path, get_standard_icon, reveal_in_file_manager
...
class FileActionHandler(QObject):
    ...
    def on_new_file(self):
        """
        Creates a new diagram file. If a project is open, it creates the file
        within the project. If no project is open, it creates a new, empty,
        unsaved tab.
        """
        if self.mw.project_manager.is_project_open():
            ...
            # No project is open, so just create a new empty tab
            self.mw.add_new_editor_tab()
            logger.info("New empty diagram tab created.")
    ...
    def on_sync_from_scratchpad(self):
        ...
        try:
            if format_type == "PlantUML":
                ...
                QMessageBox.critical(self.mw, "Unsupported Format", f"Cannot sync from '{format_type}'.")
                return
            ...
            if new_data and (new_data.get('states') or new_data.get('transitions')):
                ...
                QMessageBox.warning(self.mw, "Parsing Failed", f"Could not extract a valid FSM from the provided {format_type} text.")

        except Exception as e:
            QMessageBox.critical(self.mw, "Parsing Error", f"Failed to parse the diagram text: {e}")
    ...
    def on_project_explorer_context_menu(self, point):
        ...
        # --- MODIFIED: Use the robust cross-platform helper ---
        reveal_action.triggered.connect(lambda: reveal_in_file_manager(file_path))
        ...
        if ok and new_name and new_name != old_name:
            ...
            try:
                ...
                if editor:
                    ...
                    self.mw.ui_manager.update_window_title()
            except OSError as e:
