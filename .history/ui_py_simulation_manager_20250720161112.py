...
class PySimulationUIManager(QObject):
    ...
    def _highlight_sim_active_state(self, state_name_to_highlight: str | None):
        # <<< FIX: Delegate all highlighting to the AnimationManager >>>
        if self.animation_manager:
            if state_name_to_highlight:
                self.animation_manager.animate_state_entry(state_name_to_highlight)
            else:
    ...
    def update_dock_ui_contents(self):
        ...
        # <<< FIX: The animation manager handles the highlighting, not this direct call >>>
    ...
    def append_to_action_log(self, log_entries: list[str]):
        ...
        # <<< FIX: Trigger animations by parsing log messages >>>
    ...
    def on_reset_py_simulation(self):
        ...
        self.real_time_timer.stop()
        self._highlight_sim_active_state(None) # <<< FIX: Clear highlight on reset
        try:
