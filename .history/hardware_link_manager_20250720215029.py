    ...
    def write_data(self, data: str):
        ...
        if self.serial_connection and self.serial_connection.is_open:
            try:
                ...
                logger.debug(f"HIL TX: {data.strip()}")
            except serial.SerialException as e:
...
class HardwareLinkManager(QObject):
    ...
    hardwareDataReceived = pyqtSignal(str, object) # component_name, value
    hardwareLinkLost = pyqtSignal()
    rawDataReceived = pyqtSignal(str) # For Serial Monitor UI
    ...
    def _parse_incoming_data(self, line: str):
        ...
        self.rawDataReceived.emit(line)

        # Protocol: TYPE:<NAME>:<PAYLOAD>
        parts = line.split(':', 2)
        if len(parts) < 2:
            return
        ...
        if msg_type == "EVT":
            ...
            payload_str = parts[2]
            try:
                # Attempt to convert payload to a number (float or int)
