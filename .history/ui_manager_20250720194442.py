...
class UIManager(QObject): 
    ...
    def _create_menus(self):
        # This method is now intentionally left empty as the ribbon handles all menu functionality.
        # We keep it for structural consistency in case a traditional menu is ever needed again.
        pass

    def _create_ribbon(self):
        """Creates the new ribbon-style toolbar."""
        mw = self.mw
        # Remove any existing toolbars and the main menubar
        ...
        # Keep the View->Toolbars menu for showing/hiding the ribbon
        if not hasattr(mw, 'toolbars_menu'):
            # Create a placeholder View menu if it doesn't exist from a previous setup
            if not hasattr(mw, 'view_menu'):
                 # This logic is a failsafe. Ideally, the menu should be created elsewhere
                 # if it's meant to exist.
                 if mw.menuBar():
                     mw.view_menu = mw.menuBar().addMenu("&View")
                 else:
                     # If there's no menubar at all, we can't add the menu.
                     # This scenario should be handled based on application design.
                     # For now, we'll log a warning and skip adding the toolbar toggle.
                     logger.warning("No menu bar found to add 'View' menu for toolbar toggling.")
                     mw.view_menu = None
            if mw.view_menu:
                mw.toolbars_menu = mw.view_menu.addMenu("Toolbars")
        
        if hasattr(mw, 'toolbars_menu') and mw.toolbars_menu is not None:
             mw.toolbars_menu.clear()
             mw.toolbars_menu.addAction(ribbon_container_toolbar.toggleViewAction())

        # --- Create File Menu for Ribbon's File Button ---
        file_menu = QMenu(mw)
        ...
        # --- FIX: Create actions and assign them to the main window instance ---
        ...
        align_btn.setDefaultAction(mw.align_left_action) # Set default for tooltip
        ...
        dist_btn.setDefaultAction(mw.distribute_h_action) # Set default for tooltip
    ...
    def _create_docks(self):
        ...
        # --- FIX: Check for view_menu before clearing docks_menu ---
        if not hasattr(mw, 'view_menu'):
            mw.view_menu = QMenu("&View", mw) # Failsafe if it wasn't created
