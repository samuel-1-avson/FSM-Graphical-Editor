# fsm_designer_project/action_handlers.py
import os
import json
import logging
...
from .dialogs import ImportFromTextDialog
...
class ActionHandler(QObject):
    ...
    def on_generate_c_code(self):
        ...
        if output_dir:
            ...
            # 2. Data Types Dropdown
            data_type_combo = QComboBox()
            data_type_combo.addItems(["int", "uint16_t", "uint8_t"]) # Order from largest to smallest
            ...
            if dialog.exec() == QDialog.Accepted:
                ...
                if not filename_base[0].isalpha():
                ...
                # --- NEW: Collect options from the new widgets ---
                generation_options = {
                ...
                try:
                    ...
                    # --- NEW: Pass options to the generator function ---
