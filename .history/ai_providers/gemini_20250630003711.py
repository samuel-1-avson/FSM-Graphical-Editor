...
class GeminiProvider(AIProvider):
    ...
    def configure(self, api_key: str) -> bool:
        ...
        try:
            ...
            # Client is now created on-demand in generate_response to include system prompt
    ...
    def generate_response(self, conversation_history: List[Dict], is_json_mode: bool) -> str:
        ...
        for msg in conversation_history:
            ...
            elif msg.get("content"): # Ensure message content is not empty
                # Convert 'assistant' to 'model' for Gemini
                role = "model" if msg["role"] == "assistant" else msg["role"]
                messages.append({"role": role, "parts": [Part.from_text(msg["content"])]})

        # Re-create the model client if the system instruction has changed
        if system_prompt != self._system_instruction or self.client is None:
