...
class PySimulationUIManager(QObject):
    ...
    def update_dock_ui_contents(self):
        editor = self.mw.current_editor()
        # <<< FIX: Check for editor existence AND its simulation state >>>
        ...
        # <<< FIX: All subsequent logic correctly uses the editor's engine >>>
    ...
    def append_to_action_log(self, log_entries: list[str]):
        # This function doesn't need changes as it's just a display method.
        # However, the animation trigger logic within it must get the correct editor.
        ...
        # <<< FIX: Get current editor for animation context >>>
        editor = self.mw.current_editor()

        # --- ANIMATION TRIGGER LOGIC (VIA LOG PARSING) ---
        if editor and self.animation_manager:
            for entry in log_entries:
                ...
                if match_trans:
                    ...
                    # <<< FIX: Use editor's scene >>>
                    source_item = editor.scene.get_state_by_name(source)
                    target_item = editor.scene.get_state_by_name(target)
                    if source_item and target_item:
                        # self.animation_manager.animate_transition(source_item, target_item, event)
                        self.animation_manager.animate_state_exit(source)
                        self.animation_manager.animate_transition(source, target, event)

                # Regex to find state entry. Catches "Entering state: Idle" and "Entering state: Processing (SubIdle)"
                match_entry = re.search(r"Entering state: ([^\s(]+)", entry)
                if match_entry:
                    state_name = match_entry.group(1)
                    # <<< FIX: Use editor's scene >>>
                    state_item = editor.scene.get_state_by_name(state_name)
                    if state_item:
                        self.animation_manager.animate_state_entry(state_item)

        # Log Display Logic (unchanged but context provided)
        for entry in log_entries:
    ...
    def on_sim_variable_changed(self, item: QTableWidgetItem):
        # <<< FIX: Get current editor and its engine >>>
        editor = self.mw.current_editor()
    ...
    def on_start_py_simulation(self):
        # <<< FIX: Operate on the current editor >>>
        editor = self.mw.current_editor();
        ...
        try:
            # <<< FIX: Assign the new engine to the EDITOR, not the main window >>>
    ...
    # <<< FIX: Add a new optional argument to allow triggering from hardware widgets >>>
    @pyqtSlot()
