...
class MainWindow(QMainWindow):
    ...
    def _update_properties_dock(self):
        ...
        if show_editor and hasattr(item_to_edit, 'get_data'):
            ...
            for prop_info in schema:
                ...
                # BUG FIX: Renamed 'config' to 'widget_config' to avoid shadowing the 'config' module
                if widget_config := prop_info.get('config'):
                    for method_name, args in widget_config.items():
                        getattr(editor_widget, method_name)(*args) if isinstance(args, tuple) else getattr(editor_widget, method_name)(args)
    ...
    def on_matlab_settings(self, checked=False): 
        from .dialogs import MatlabSettingsDialog # Local import to avoid circular dependency
