...
# REMOVED: from .ai_chatbot import AIChatUIManager
# This import created a circular dependency with ai_chatbot.py, which needs to import dialogs itself.
# The type hint will be changed to a string literal to resolve this.
...
class StatePropertiesDialog(QDialog):
    # --- BUG-01 FIX: Add 'ai_chat_ui_manager' to the constructor, now with a string type hint ---
    def __init__(self, settings_manager: SettingsManager, custom_snippet_manager: CustomSnippetManager, ai_chat_ui_manager:     'AIChatUIManager | None', parent=None, current_properties=None, is_new_state=False, scene_ref=None):
        super().__init__(parent)
        ...
        self.ai_chat_ui_manager = ai_chat_ui_manager # <-- BUG-01 FIX: STORE THE MANAGER
        # --- END DI ---

        # self.parent_window_ref = parent # This is no longer needed for the AI helper
    ...
    # --- BUG-01 FIX: AI Helper Button Creation ---
    ...
    # --- BUG-01 FIX: Simplified AI helper method ---
    def _on_ai_helper_clicked(self, target_widget, code_type):
        ...
        # Use the stored manager directly - no more parent-climbing!
        if not self.ai_chat_ui_manager:
            QMessageBox.warning(self, "AI Not Available", "The AI assistant UI manager was not provided to this dialog.")
            return
        ...
        # Call the handler method on the stored manager
        self.ai_chat_ui_manager.handle_inline_ai_request(prompt, language)
...
# In fsm_designer_project/dialogs.py

# ... (other imports and classes) ...

class TransitionPropertiesDialog(QDialog):
    # --- FIX: Added settings_manager to the constructor for consistency and future use ---
    def __init__(self, settings_manager: SettingsManager, custom_snippet_manager: CustomSnippetManager, ai_chat_ui_manager: 'AIChatUIManager | None', parent=None, current_properties=None, is_new_transition=False):
        super().__init__(parent)
        ...
        # --- DEPENDENCY INJECTION ---
        self.settings_manager = settings_manager # <-- FIX: Store the settings manager
        self.custom_snippet_manager = custom_snippet_manager
        self.ai_chat_ui_manager = ai_chat_ui_manager
        # --- END DI ---
        
        main_layout = QVBoxLayout(self)
        main_layout.setSpacing(10)
        main_layout.setContentsMargins(12,12,12,12) # FIX: Corrected margins tuple
        p = current_properties or {}
        ...
        # --- AI Helper buttons for transition ---
        ...
        self.action_edit = CodeEditor()
        self.action_edit.setPlainText(p.get('action', ""))
        self.action_edit.setFixedHeight(80)
        self.action_edit.setObjectName("ActionCodeEditor") 
        ...
        def add_codeeditor_with_snippet_and_hw_hint(form_layout, label_text, code_editor_widget, snippet_button, ai_button):
            ...
            hw_hint_label.setObjectName("HardwareHintLabel")
            hw_hint_label.setStyleSheet(f"color: {COLOR_TEXT_SECONDARY}; font-style: italic; font-size: 7.5pt;")
        ...
        self.offset_perp_spin.setSuffix(" px")
        ...
        self.offset_tang_spin.setSuffix(" px")
        curve_layout = QHBoxLayout()
        curve_layout.addWidget(QLabel("Bend (Perp):"))
        curve_layout.addWidget(self.offset_perp_spin,1)
        curve_layout.addSpacing(15)
        curve_layout.addWidget(QLabel("Mid Shift (Tang):"))
        curve_layout.addWidget(self.offset_tang_spin,1)
        ...
        self.line_width_spin.setValue(p.get('line_width', DEFAULT_TRANSITION_LINE_WIDTH))
    ...
    # ... (rest of the TransitionPropertiesDialog class methods)

    # --- BUG-01 FIX: AI Helper button creation and handler ---
    def _create_ai_helper_button(self, target_widget, code_type="action"):
        button = QPushButton()
    ...
    def _on_ai_helper_clicked(self, target_widget, code_type):
        ...
        # Use the stored manager directly
        if not self.ai_chat_ui_manager:
            QMessageBox.warning(self, "AI Not Available", "The AI assistant manager was not provided to this dialog.")
            return
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, settings_manager: SettingsManager, theme_manager: ThemeManager, parent=None):
        ...
        self.edit_theme_btn = QPushButton("Edit..."); self.edit_theme_btn.clicked.connect(self._on_edit_theme)
        self.delete_theme_btn = QPushButton("Delete"); self.delete_theme_btn.clicked.connect(self._on_delete_theme)
    ...
    def on_add_snippet(self):
        ...
        if dialog.exec_():
            ...
            if self.snippet_manager.add_custom_snippet(language, category, data["name"], data["code"]):
                self.on_selection_changed() # Refresh list
            else:
...
# --- NEW: AiSettingsDialog (Moved from ai_chatbot.py) ---
