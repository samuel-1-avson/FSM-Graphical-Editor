...
from .core.application_context import ApplicationContext  # <-- NEW IMPORT
...
from PyQt6.QtCore import QThread, QMetaObject, Q_ARG
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, context: ApplicationContext):
        ...
        # Managers that depend on `self` (the QMainWindow) are still created here
        self.project_manager = ProjectManager(self)
        self.context.project_manager = self.project_manager # Store back in context if needed elsewhere
        ...
        # --- NEW: Setup for the FileLoaderWorker ---
        ...
        # These will temporarily store context for the async load operation
    ...
    def _create_and_load_new_tab(self, file_path: str):
        ...
        # This now triggers the asynchronous load process.
    ...
    def _load_into_editor(self, editor: EditorWidget, file_path: str):
        """Triggers an asynchronous load of a diagram file into the specified editor."""
        ...
        if schema.get("title") == "Schema Loading Failed":
            ...
            self._on_file_load_finished({}, msg) # Immediately fail
            return
    ...
    def _on_file_load_finished(self, diagram_data: dict, error_string: str):
        """Finalizes file loading after the background worker is done."""
        ...
        if error_string:
            ...
            # Clean up the empty tab created for the failed load
            index = self.tab_widget.indexOf(editor)
            if index != -1:
                self.tab_widget.removeTab(index)
            editor.deleteLater()
        else:
            # Success! Populate the editor.
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        # --- NEW: Stop the file loader thread ---
