...
class ModernRibbon(QWidget):
    ...
    def init_ui(self):
        ...
        self.tab_bar.setStyleSheet(f"background-color: {COLOR_BACKGROUND_APP};")
        ...
        self.file_button.setStyleSheet(f"""
            ...
            QPushButton:hover {{ background-color: {QColor(COLOR_ACCENT_PRIMARY).lighter(110).name()}; }}
            QPushButton:checked, QPushButton:pressed {{ background-color: {QColor(COLOR_ACCENT_PRIMARY).darker(110).name()}; }}
        """)
        ...
        self.content_area.setStyleSheet(f"background-color: {COLOR_BACKGROUND_LIGHT}; border-bottom: 1px solid {COLOR_BORDER_LIGHT};")
    ...
    def set_file_menu(self, menu: QMenu):
        ...
        self.file_button.clicked.connect(lambda: self.tab_button_group.setExclusive(False))
        self.file_button.clicked.connect(lambda: self.file_button.setChecked(True))
        self.file_button.clicked.connect(lambda: self.tab_button_group.setExclusive(True))
        menu.aboutToHide.connect(self.file_button.toggle)
