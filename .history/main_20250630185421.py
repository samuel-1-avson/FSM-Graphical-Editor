...
class MainWindow(QMainWindow):
    ...
    def _on_git_status_updated(self, file_path: str, is_in_repo: bool, has_changes: bool):
        ...
        if is_in_repo and has_changes:
            icon = get_standard_icon(QStyle.SP_VCS_Update, "Git[M]")
    ...
    def _update_all_ui_element_states(self):
        ...
        self._request_live_preview_update() # Add this line
    ...
    def _update_window_title(self):
        ...
        pysim_suffix = f" [PySim Active in '{editor.get_tab_title().strip()}' tab]" if editor.py_sim_active else ""
    ...
    def _request_live_preview_update(self):
        """Debounces requests to update the live preview dock."""
        if hasattr(self, 'live_preview_dock') and self.live_preview_dock.isVisible():
            self.live_preview_update_timer.start()

    @pyqtSlot()
    def _update_live_preview(self):
        """Generates and displays code in the live preview dock."""
        editor = self.current_editor()
        if not editor or not hasattr(self, 'live_preview_dock') or not self.live_preview_dock.isVisible():
            return

        diagram_data = editor.scene.get_diagram_data()
        lang = self.live_preview_combo.currentText()
        code = ""
        highlight_lang = "Text"

        try:
            if not diagram_data.get('states'):
                code = "# Add some states to the diagram to generate code."
            elif lang == "Python FSM":
                base_name = os.path.splitext(os.path.basename(editor.file_path or "my_fsm"))[0]
                class_name = "".join(word.capitalize() for word in base_name.replace('-', '_').split('_')) or "MyFsm"
                code = generate_python_fsm_code(diagram_data, class_name)
                highlight_lang = "Python"
            elif lang == "C Code":
                base_name = os.path.splitext(os.path.basename(editor.file_path or "my_fsm"))[0]
                c_content = generate_c_code_content(diagram_data, base_name)
                code = f"// --- HEADER FILE: {base_name}.h ---\n\n"
                code += c_content['h']
                code += f"\n\n// --- SOURCE FILE: {base_name}.c ---\n\n"
                code += c_content['c']
                highlight_lang = "C/C++ (Generic)"
            elif lang == "PlantUML":
                base_name = os.path.splitext(os.path.basename(editor.file_path or "fsm_diagram"))[0]
                code = generate_plantuml_text(diagram_data, base_name)
                highlight_lang = "Text" # No specific highlighter for PlantUML yet
            elif lang == "Mermaid":
                base_name = os.path.splitext(os.path.basename(editor.file_path or "fsm_diagram"))[0]
                code = generate_mermaid_text(diagram_data, base_name)
                highlight_lang = "Text" # No specific highlighter for Mermaid yet

        except Exception as e:
            logger.error(f"Error generating live preview for {lang}: {e}", exc_info=True)
            code = f"# Error generating preview for {lang}:\n# {e}"
        
        if hasattr(self, 'live_preview_editor'):
            self.live_preview_editor.setPlainText(code)
            self.live_preview_editor.set_language(highlight_lang)
