...
from PyQt5.QtGui import (
    QIcon, QKeySequence, QCloseEvent, QPalette, QColor, QPen, QFont
)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._find_dialogs = {}
        # *** FIX: Initialize timer before it's used in signal connections ***
        ...
        # self.setCentralWidget(self.tab_widget) # DEFERRED: Welcome page logic
        ...
        # MODIFIED: Don't create a new file if no tabs, let welcome page show
    ...
    def _update_central_widget(self):
        ...
        if self.tab_widget.count() == 0:
            if self.centralWidget() != self.welcome_widget:
                ...
                self.welcome_widget.update_recent_files() # Refresh recent files list
        else:
    ...
    def _connect_application_signals(self):
        ...
        # --- NEW: Connect AI helper for validation issues ---
    ...
    # update_problems_dock = lambda self, *args: None # MODIFIED: This is now fully implemented
    ...
    def _apply_theme(self, theme_name: str):
        ...
        # --- NEW: Update welcome page colors ---
    ...
    def _connect_editor_signals(self, editor: EditorWidget):
        ...
        # NEW connections for live preview
    ...
    def add_new_editor_tab(self) -> EditorWidget:
        ...
        # Pass dependencies to the scene
        ...
        self._update_window_title()
        self._update_central_widget() # --- NEW: Ensure correct central widget is shown
    ...
    def _update_live_preview(self):
        ...
        This could generate PlantUML, Mermaid, or code previews.
        """
        editor = self.current_editor()
        if not editor or not hasattr(self, 'live_preview_dock') or not self.live_preview_dock.isVisible():
            return

        # Example: Update PlantUML or Mermaid preview based on selected option
        preview_type = None
        if hasattr(self, 'live_preview_combo'):
        ...
        if preview_type == "PlantUML":
            from .export_utils import generate_plantuml_text
            preview_text = generate_plantuml_text(diagram_data) if diagram_data else ""
        elif preview_type == "Mermaid":
            from .export_utils import generate_mermaid_text
            preview_text = generate_mermaid_text(diagram_data) if diagram_data else ""
        elif preview_type == "C Code":
            from .c_code_generator import generate_c_code_content
            # This returns a dict, so we need to format it for display
            if diagram_data and diagram_data['states']:
                ...
                preview_text = "// No states to generate C code."
        elif preview_type == "Python FSM":
            from .python_code_generator import generate_python_fsm_code
            preview_text = generate_python_fsm_code(diagram_data, "LivePreviewFSM") if diagram_data and diagram_data['states'] else "# No states to generate Python code."
        else:
            preview_text = "No preview available for this type."

        # Set the preview text in the appropriate widget
        if hasattr(self, 'live_preview_editor'):
            # Temporarily block signals to avoid recursive updates if editor emits textChanged
    ...
    def _request_live_preview_update(self):
        ...
        This debounces rapid changes and ensures the preview updates after a short delay.
        """
    ...
    def _update_matlab_actions_enabled_state(self):
        can_run_matlab_ops = self.matlab_connection.connected and not (self.current_editor() and self.current_editor().py_sim_active)
        ...
        if hasattr(self, 'matlab_settings_action'): self.matlab_settings_action.setEnabled(not (self.current_editor() and self.current_editor().py_sim_active))
