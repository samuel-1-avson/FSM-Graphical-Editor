...
class FSMSimulator:
    ...
    def step(self, event_name=None):
        ...
        for trans in self.transitions:
            if trans['source'] == current_state_in_path['name'] and trans['event'] == event_name:
                condition_ok = True
                if trans.get('condition'):
                    try:
                        ...
                        condition_ok = False
                if condition_ok:
                    self.log_action(f"After transition on '{event_name}' from '{current_state_in_path['name']}' to '{trans['target']}'")
                    ...
                    if new_state_data:
                        self.current_state_path.pop() # Exit current sub-state
