...
def generate_c_code_content(diagram_data: dict, base_filename: str) -> dict[str, str]:
    """
    Generates C header and source content as strings from diagram data.
    Returns a dictionary with 'h' and 'c' content.
    """
...
def generate_c_code_files(diagram_data: dict, output_dir: str, base_filename: str) -> tuple[str, str]:
    ...
    """
    try:
        content = generate_c_code_content(diagram_data, base_filename)
        h_code = content['h']
        c_code = content['c']
        ...
        with open(h_file_path, 'w', encoding='utf-8') as f: f.write(h_code)
        with open(c_file_path, 'w', encoding='utf-8') as f: f.write(c_code)
