...
from ...managers.snippet_manager import CustomSnippetManager
from ...codegen.fsm_importer import parse_plantuml, parse_mermaid
from ...assets.assets import MECHATRONICS_SNIPPETS
from ...codegen.config import APP_NAME, APP_VERSION, COLOR_BACKGROUND_DIALOG, COLOR_ACCENT_PRIMARY, COLOR_TEXT_PRIMARY, COLOR_BORDER_LIGHT, COLOR_BACKGROUND_LIGHT
...
class SnippetManagerDialog(QDialog):
    ...
    def on_rename_template(self):
        ...
        if ok and new_name.strip() and new_name.strip() != old_name:
            ...
            if self.snippet_manager.template_exists(new_name):
                QMessageBox.warning(self, "Name Exists", f"A template named '{new_name}' already exists.")
                return

            if self.snippet_manager.rename_custom_template(old_name, new_name):
                self.on_template_selection_changed()
            else:
