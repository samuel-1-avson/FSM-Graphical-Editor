...
from ...codegen.fsm_importer import parse_plantuml, parse_mermaid
...
class SnippetManagerDialog(QDialog):
    ...
    def _create_template_tab(self):
        ...
        self.template_preview.setPlaceholderText("Select a custom template to manage it.")
        
        right_layout.addLayout(template_button_layout) # Add the button layout
    ...
    def on_selection_changed(self):
        ...
        snippet_names = self.asset_manager.get_snippet_names_for_language_category(language, category)
    ...
    def on_snippet_selected(self, current_item: QListWidgetItem, previous_item: QListWidgetItem = None):
        ...
        if is_item_selected:
            ...
            language = self.lang_combo.currentText()
            category = self.cat_combo.currentText()
            code = self.asset_manager.get_snippet_code(language, category, name)
    ...
    def on_add_snippet(self):
        ...
        if dialog.exec():
            ...
            if self.asset_manager.add_custom_snippet(language, category, data["name"], data["code"]):
                self.on_selection_changed()
            else:
    ...
    def on_edit_snippet(self):
        ...
        language = self.lang_combo.currentText()
        category = self.cat_combo.currentText()
        old_code = self.asset_manager.get_snippet_code(language, category, old_name)
        dialog = SnippetEditDialog(self, language, category, old_name, old_code)
        if dialog.exec():
            data = dialog.get_snippet_data()
            if self.asset_manager.edit_custom_snippet(language, category, old_name, data["name"], data["code"]):
                self.on_selection_changed()
            else:
    ...
    def on_delete_snippet(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if self.asset_manager.delete_custom_snippet(language, category, name):
                self.on_selection_changed()
            else:
    ...
    def on_template_selection_changed(self):
        ...
        templates = self.asset_manager.get_custom_templates()
        ...
        self.rename_template_button.setEnabled(is_item_selected)
    ...
    def on_rename_template(self):
        current_item = self.template_list_widget.currentItem()
        if not current_item: return
        
        old_name = current_item.text()
        
        new_name, ok = QInputDialog.getText(self, "Rename Template", "Enter new name for the template:", text=old_name)
        
        if ok and new_name.strip() and new_name.strip() != old_name:
            new_name = new_name.strip()
            if self.asset_manager.template_exists(new_name):
                QMessageBox.warning(self, "Name Exists", f"A template named '{new_name}' already exists.")
                return

            if self.asset_manager.rename_custom_template(old_name, new_name):
                self.on_template_selection_changed()
            else:
                QMessageBox.critical(self, "Rename Error", "Failed to rename the template.")
    ...
    def on_delete_template(self):
        ...
        reply = QMessageBox.question(self, "Delete Template", f"Are you sure you want to delete the FSM template '{name}'?",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            if self.asset_manager.delete_custom_template(name):
                self.on_template_selection_changed()
            else:
...
class SystemInfoDialog(QDialog):
    ...
    def _gather_info_as_html(self) -> str:
        ...
        try:
            ...
            html = f"""
            <style>
                body {{ font-family: Segoe UI, sans-serif; background-color: {theme_config.COLOR_BACKGROUND_LIGHT}; color: {theme_config.COLOR_TEXT_PRIMARY}; }}
                h2 {{ color: {theme_config.COLOR_ACCENT_PRIMARY}; border-bottom: 1px solid {theme_config.COLOR_BORDER_LIGHT}; padding-bottom: 4px; }}
