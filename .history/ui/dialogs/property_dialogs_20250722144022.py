...
class StatePropertiesDialog(QDialog):
    ...
    def _on_ai_helper_clicked(self, target_widget, code_type):
        ...
        if not main_win or not hasattr(main_win, 'ai_chat_ui_manager') or not hasattr(main_win, 'ai_chatbot_manager'):
        ...
        # --- NEW: Inline AI request logic ---
        # 1. Connect a one-shot signal to receive the response
        # 2. Send the request to the main chatbot manager
        
        def on_inline_response(code_snippet, source_prompt, request_id):
            # This slot will receive the generated code
            logger.info(f"Inline response received for widget {target_widget.objectName()}.")
            ...
            # Insert the code into the target widget
            target_widget.insertPlainText(code_to_insert)

            # Disconnect to avoid handling future responses
            try:
                main_win.ai_chatbot_manager.inlineResponseReady.disconnect(on_inline_response)
            except TypeError:
                logger.warning("Could not disconnect inline AI response handler.")

        # Disconnect any previous handlers to be safe
        try:
            main_win.ai_chatbot_manager.inlineResponseReady.disconnect()
        except TypeError:
            pass # No connections to disconnect
            
        main_win.ai_chatbot_manager.inlineResponseReady.connect(on_inline_response)
        
        # Use a unique ID for the request (though not strictly needed for this one-shot connection)
        request_id = f"dialog_req_{id(target_widget)}"
        main_win.ai_chatbot_manager.generate_inline_code_snippet(prompt, request_id)

        # Show a notification to the user
        main_win.ai_chat_ui_manager._append_to_chat_display("You (Inline Request)", prompt)
        main_win.ai_chat_ui_manager._append_to_chat_display("System", "Generating code snippet... The result will be inserted here and in the main chat.")
