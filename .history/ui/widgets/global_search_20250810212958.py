...
class SearchResultItemWidget(QWidget):
    ...
    def __init__(self, icon, text, subtitle, parent=None):
        ...
        from ...utils.config import COLOR_TEXT_PRIMARY, COLOR_TEXT_SECONDARY, APP_FONT_SIZE_SMALL
        ...
        self.title_label.setStyleSheet(f"color: {COLOR_TEXT_PRIMARY}; font-weight: 500;")
        text_layout.addWidget(self.title_label)

        if subtitle:
            self.subtitle_label = QLabel(subtitle)
            self.subtitle_label.setStyleSheet(f"color: {COLOR_TEXT_SECONDARY}; font-size: {APP_FONT_SIZE_SMALL};")
...
class GlobalSearchHandler(QWidget):
    ...
    def _init_ui(self):
        from ...utils.config import COLOR_BACKGROUND_LIGHT, COLOR_ACCENT_PRIMARY_LIGHT, COLOR_ACCENT_PRIMARY, COLOR_BORDER_MEDIUM
