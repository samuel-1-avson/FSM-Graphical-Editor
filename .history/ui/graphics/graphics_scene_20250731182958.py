...
# --- FIX: Defer these imports to break the circular dependency ---
...
class DiagramScene(QGraphicsScene):
    ...
    item_edit_requested = pyqtSignal(QGraphicsItem) # <-- NEW SIGNAL

    def __init__(self, undo_stack, parent_window=None, custom_snippet_manager = None, settings_manager = None):
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        # This method is now called by MainWindow in response to the signal,
        # but the logic inside it remains correct.
    ...
    def _show_item_context_menu(self, item: QGraphicsItem, event: QGraphicsSceneContextMenuEvent):
        ...
        menu.addAction(get_standard_icon(QStyle.SP_DialogApplyButton, "Edt"), "&Properties...", lambda: self.item_edit_requested.emit(item))
        ...
        if self.parent_window and self.parent_window.current_editor() and self.parent_window.current_editor().py_sim_active:
            if isinstance(item, GraphicsStateItem):
                ...
                # New transition breakpoint logic
                bp_action = QAction("Toggle Transition Breakpoint", menu, checkable=True)
                bp_action.setIcon(get_standard_icon(QStyle.SP_DialogYesButton, "BP"))
                
                # Check if this transition has a breakpoint
    ...
    def mouseDoubleClickEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        # --- MODIFIED: Emit signal instead of calling edit_item_properties ---
        if item_to_edit:
