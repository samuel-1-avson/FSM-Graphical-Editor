...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        if not self._is_editing_inline:
            ...
            painter.drawText(text_rect, Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.TextWordWrap, self.text_label) # <--- CORRECTED
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.GraphicsItemFlag.ItemPositionHasChanged and self.scene(): self.scene().item_moved.emit(self)
...
class GraphicsTransitionItem(QGraphicsPathItem):
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        if current_label:
            from PyQt6.QtGui import QFontMetrics; painter.setFont(self._font); fm = QFontMetrics(self._font) 
            label_rect_width = 150; text_block_rect = QRectF(0, 0, label_rect_width, 100); text_rect_original = fm.boundingRect(text_block_rect.toRect(), Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.TextWordWrap, current_label)
            ...
            painter.setPen(self._text_color); painter.drawText(text_final_draw_rect, Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.TextWordWrap, current_label)
    ...
    def _get_intersection_point(self, item: QGraphicsRectItem, line: QLineF): 
        ...
        for edge in edges:
            ...
            if intersect_type == QLineF.Intersecting: intersect_points.append(QPointF(intersection_point_var))
...
class GraphicsCommentItem(QGraphicsTextItem):
    ...
    def itemChange(self, change, value): 
        if change == QGraphicsItem.GraphicsItemFlag.ItemPositionHasChanged and self.scene(): self.scene().item_moved.emit(self)
        elif change == QGraphicsItem.GraphicsItemFlag.ItemSelectedHasChanged and not value and self._is_editing_inline and self._inline_editor_proxy and self._inline_editor_proxy.widget():
