...
class TransitionPulseItem(QGraphicsObject):
    ...
    def __init__(self, parent_transition):
        ...
        glow_color = QColor(theme_config.COLOR_ACCENT_SECONDARY).lighter(120)
...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        if self._is_potential_transition_target and not self.isSelected() and not self.is_py_sim_active:
            highlight_color = QColor(theme_config.COLOR_ACCENT_PRIMARY)
        ...
        if self.is_initial:
            marker_radius = 7; line_length = 20; marker_color = QColor(theme_config.COLOR_TEXT_PRIMARY) 
        ...
        if self.isSelected() and not self.is_py_sim_active:
            ...
            selection_pen = QPen(QColor(theme_config.COLOR_ITEM_STATE_SELECTION_BORDER), selection_pen_width, Qt.PenStyle.DashLine)
            selection_brush_color = QColor(theme_config.COLOR_ITEM_STATE_SELECTION_BG); selection_brush_color.setAlpha(80)
...
class GraphicsTransitionItem(QGraphicsPathItem):
    ...
    def _get_actual_control_point_scene_pos(self) -> QPointF: 
        ...
        if self.start_item == self.end_item:
            rect = self.start_item.sceneBoundingRect(); p1_scene = QPointF(rect.center().x() + rect.width() * 0.2, rect.top()); p2_scene = QPointF(rect.center().x() - rect.width() * 0.2, rect.top()); loop_radius_y = rect.height() * 0.55
...
class GraphicsCommentItem(QGraphicsTextItem):
    ...
    def __init__(self, x, y, text="Comment",
                 font_family=None, font_size=None, font_italic=None, bg_color=None):
        super().__init__()
        self.setHtml(text)
        self.setPos(x, y)
        ...
        default_color_hex = bg_color or (settings.get("item_default_comment_bg_color") if settings else theme_config.COLOR_ITEM_COMMENT_BG)
    ...
    def set_properties(self, **props):
        ...
        if current_actual_width < 0:
            current_actual_width = self.document().idealWidth() if self.document() else self._default_width
        effective_target_width = target_width if target_width > 0 else current_actual_width
        if abs(current_actual_width - effective_target_width) > 1e-3:
            width_changed = True
            changed = True

        if width_changed or (text_changed and target_width < 0):
        ...
        if bg_color is not None:
            new_color_obj = QColor(bg_color)
            if new_color_obj.isValid() and self.background_brush.color() != new_color_obj:
                self.background_brush.setColor(new_color_obj)
                self.original_border_pen.setColor(new_color_obj.darker(110))
                self.border_pen = QPen(self.original_border_pen)
                changed = True
    ...
    def get_data(self):
        ...
        return {
            ...
            'bg_color': self.background_brush.color().name()
        }
    def set_problematic_style(self, is_problematic: bool, problem_description: str = ""):
        if self._is_problematic == is_problematic and self._problem_tooltip_text == problem_description: return
        self._is_problematic = is_problematic; self._problem_tooltip_text = problem_description if is_problematic else ""
...
class GraphicsFrameItem(QGraphicsRectItem):
    """A visual frame to group other items, with advanced parenting and interaction."""
    ...
    def __init__(self, x, y, w, h, title="Group"):
        ...
        # --- NEW: Store child items and their relative positions ---
        self._child_items = []
        self._child_offsets = {}
        
        # --- NEW: Store previous position for move calculations ---
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        # Draw title bar with gradient for a professional look
        title_rect = QRectF(0, 0, self.rect().width(), 24)
        gradient = QLinearGradient(title_rect.topLeft(), title_rect.bottomLeft())
        gradient.setColorAt(0, QColor(theme_config.COLOR_BACKGROUND_DARK).lighter(110))
        gradient.setColorAt(1, QColor(theme_config.COLOR_BACKGROUND_DARK))
        painter.setBrush(gradient)
        painter.setPen(Qt.PenStyle.NoPen)
        path = QPainterPath()
        path.addRoundedRect(title_rect, 10, 10)
        path.addRect(QRectF(0, 12, self.rect().width(), 12)) # Make bottom corners square
        ...
        text_rect = title_rect.adjusted(10, 0, -10, 0)
    ...
    def itemChange(self, change, value):
        """Override to move child items when the frame is moved."""
        if change == QGraphicsItem.GraphicsItemChange.ItemPositionChange and self.scene():
            # Calculate the delta move
            move_delta = value - self.pos()
            
            # Move all child items by the same delta
            for child in self._child_items:
                if not child.isSelected(): # Avoid double-moving if child is also selected
                    child.moveBy(move_delta.x(), move_delta.y())
                    
        return super().itemChange(change, value)

    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        """Capture child items and their offsets when starting a move."""
        super().mousePressEvent(event)
        if event.button() == Qt.MouseButton.LeftButton:
            self._child_items.clear()
            self._child_offsets.clear()
            
            colliding_items = self.collidingItems()
            for item in colliding_items:
                if isinstance(item, (GraphicsStateItem, GraphicsCommentItem, GraphicsTransitionItem)):
                    # Check if the item is visually contained
                    if self.sceneBoundingRect().contains(item.sceneBoundingRect()):
                        self._child_items.append(item)
                        self._child_offsets[item] = item.pos() - self.pos()

    def mouseDoubleClickEvent(self, event: QGraphicsSceneMouseEvent):
        """Fit the frame to its content on double-click."""
        self.fit_to_content()
        event.accept()

    def fit_to_content(self):
        """Resizes the frame to fit all items visually contained within it."""
        from ...undo_commands import EditItemPropertiesCommand
        
        content_rect = QRectF()
        
        # Find all items inside the current frame
        for item in self.scene().items(self.sceneBoundingRect()):
            if item is self or not isinstance(item, (GraphicsStateItem, GraphicsCommentItem)):
                continue
        ...
        if not content_rect.isEmpty():
            padding = 20
            new_rect = content_rect.adjusted(-padding, -padding - 24, padding, padding) # Extra padding at top for title
            ...
            # Create an undo command for the resize
            cmd = EditItemPropertiesCommand(self, old_props, new_props, "Fit Frame to Content")
            self.scene().undo_stack.push(cmd)
