...
class ModernGroupBox(QGroupBox):
    """Enhanced GroupBox with modern, minimal styling"""
...
class StatusIndicator(QLabel):
    """Simplified status indicator with solid colors"""
...
class ModernButton(QPushButton):
    """Button with clean, professional styling"""
...
class ModernSlider(QSlider):
    """Simplified slider with minimal design"""
...
class PySimulationUIManager(QObject):
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        self.py_sim_start_btn.setStyleSheet("""
            QToolButton {
                background-color: #007AFF;
                color: #FFFFFF;
                border: none;
        ...
        self.py_sim_stop_btn.setStyleSheet("""
            QToolButton {
                background-color: #007AFF;
                color: #FFFFFF;
                border: none;
        ...
        self.py_sim_reset_btn.setStyleSheet("""
            QToolButton {
                background-color: #007AFF;
                color: #FFFFFF;
                border: none;
        ...
        self.py_sim_speed_label.setStyleSheet("font-weight: 500; color: #666666;")
        ...
        self.py_sim_current_tick_label.setStyleSheet("""
            font-size: 10pt;
            color: #666666;
            padding: 4px 8px;
            background-color: #F5F5F5;
        ...
        self.py_sim_event_combo.setStyleSheet("""
            QComboBox {
                padding: 8px 12px;
                border: 1px solid #E0E0E0;
                border-radius: 4px;
        ...
        self.py_sim_variables_table.setStyleSheet("""
            QTableWidget {
                border: 1px solid #E0E0E0;
                border-radius: 4px;
                background-color: #FFFFFF;
        ...
        self.py_sim_action_log_output.setStyleSheet("""
            QTextEdit {
                border: 1px solid #E0E0E0;
                border-radius: 4px;
                background-color: #FFFFFF;
    ...
    def update_dock_ui_contents(self):
        ...
        paused_suffix = " <span style='color:#FF8C00; font-weight:bold;'>(Paused)</span>" if editor.py_fsm_engine.paused_on_breakpoint else ""
    ...
    def append_to_action_log(self, log_entries: list[str]):
        ...
        for entry in log_entries:
            ...
            if tick_match:
                ...
                tick_str = f"<span style='color:#666666; font-size:8pt;'>[{sub_prefix}Tick {tick_num:,}]</span> "
            
            cleaned_entry = html.escape(actual_entry_msg)
            log_html = f"""
            <div style='margin:2px 0; padding:4px 8px; background:#FFFFFF;'>
                <span style='color:#666666; font-size:8pt; font-family:monospace;'>[{timestamp}]</span>
                {tick_str}
                <span style='color:#333333;'>{cleaned_entry}</span>
            </div>
            """
            if "ERROR" in entry.upper():
                log_html = log_html.replace("background:#FFFFFF;", "background:#FFEEEE;")
                log_html = log_html.replace("color:#333333;", "color:#FF0000;")
