...
class PySimulationUIManager(QObject):
    ...
    def on_toggle_matlab_simulation(self, checked):
        main_window = self.parent()
        if checked:
            if not self.matlab_sim_manager.last_generated_model_path or not os.path.exists(self.matlab_sim_manager.last_generated_model_path):
            ...
            self.matlab_sim_manager.start(self.matlab_sim_manager.last_generated_model_path)
    ...
    def on_step_py_simulation(self, internal=False):
        ...
        try:
            ...
            if editor.py_fsm_engine.simulation_halted_flag or editor.py_fsm_engine.paused_on_breakpoint:
                self.real_time_timer.stop()
                if not internal: 
                    QMessageBox.information(main_window, "Simulation Paused", "Simulation has paused due to a breakpoint or error.")
        except (FSMError, Exception) as e: 
            self.real_time_timer.stop()
