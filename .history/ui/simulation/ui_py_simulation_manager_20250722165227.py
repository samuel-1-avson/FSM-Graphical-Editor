...
class PySimulationUIManager(QObject):
    ...
    def __init__(self, main_window, action_handler, parent=None):
        ...
        # --- FIX: REMOVED FAULTY/REDUNDANT CONNECTION LOGIC ---
    ...
    # --- FIX: This entire method was redundant and called at the wrong time. It is now removed. ---
    # The ActionHandler now correctly manages these connections.
    ...
    def update_dock_ui_contents(self):
        ...
        if not editor or not editor.py_fsm_engine or not editor.py_sim_active: 
            ...
            if hasattr(self.mw, 'hardware_sim_ui_manager'):
                ...
                for gauge in hw_mgr.virtual_gauges.values(): gauge.setValue(0)
                if hw_mgr.hardware_link_manager.is_connected:
                    ...
                    for i in range(2): hw_mgr.hardware_link_manager.send_command(f"CMD:Gauge{i}:0")
            ...
            for comp_key, comp_widget in all_components.items():
                ...
                if isinstance(comp_widget, VirtualLedWidget):
                    ...
                    try:
                        ...
                        pass # Keep it 0 if conversion fails
                    ...
                    # Note: We don't send commands for sliders as they are inputs
