...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QGroupBox, QFormLayout, QLabel, 
                             QSlider, QComboBox, QPushButton, QHBoxLayout)
from PyQt5.QtCore import Qt, pyqtSlot
from .virtual_hardware_widgets import VirtualLedWidget, VirtualButtonWidget, VirtualSliderWidget
from .config import (COLOR_TEXT_SECONDARY, APP_FONT_SIZE_SMALL, COLOR_ACCENT_SUCCESS, 
                     COLOR_ACCENT_ERROR)
...
class VirtualHardwareUIManager:
    ...
    def on_virtual_button_clicked(self, button_key: str):
        """
        Handles clicks on the software buttons in the UI by simulating a hardware event.
        """
    ...
    def on_hardware_event(self, component_key: str):
        """
        Receives an event from a hardware component (real or virtual) and
        triggers the corresponding FSM event.
        """
        ...
        # Find which FSM transition this component is mapped to in the diagram
            ...
            if isinstance(item, GraphicsTransitionItem):
                ...
                if item_data.get('hw_input_map') == component_key:
                    ...
                    if triggered_event:
                        ...
                        # We found our transition, no need to check others
                        return
        
        if not triggered_event:
             logger.info(f"Hardware component '{component_key}' was activated, but it is not mapped to any transition's event in the current diagram.")