...
class AutoLayoutPreviewDialog(QDialog):
    def __init__(self, preview_pixmap: QPixmap, parent=None):
        ...
        # --- FIX: Use a button with the correct role ---
        # QDialogButtonBox.Ok has the AcceptRole by default, which emits the 'accepted' signal.
        # We can change its text to "Apply" for the user.
        button_box = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        ok_button = button_box.button(QDialogButtonBox.Ok)
        if ok_button:
            ok_button.setText("Apply")

        # This connection is now correct because the OK button will trigger it.
        button_box.accepted.connect(self.accept)
        button_box.rejected.connect(self.reject)
        
        layout.addWidget(button_box)

# Add this new class inside dialogs.py
class AiSettingsDialog(QDialog):
    """A generic dialog to select an AI provider and enter its API key."""
    def __init__(self, settings_manager, parent=None):
        super().__init__(parent)
        self.settings_manager = settings_manager
        self.setWindowTitle("AI Assistant Settings")
        self.setMinimumWidth(500)
        self.setWindowIcon(get_standard_icon(QStyle.SP_MessageBoxQuestion, "AI-Cfg"))
        ...
        self.provider_combo.addItems(sorted(self.available_providers.keys()))
        ...
        self.key_label = QLabel("API Key:") # We will change the text of this label
        layout.addRow(self.key_label, self.api_key_edit)

        # Help/Info Label
        self.info_label = QLabel("<small>API keys are stored locally and are not shared.</small>")
        ...
        current_provider = self.settings_manager.get("ai_provider_name", "Gemini (Google AI)")
        if current_provider in self.available_providers:
            self.provider_combo.setCurrentText(current_provider)
        
        # Trigger the change handler to load the correct key
        self.on_provider_changed(self.provider_combo.currentText())

    def on_provider_changed(self, provider_name: str):
        """Update UI when a new provider is selected."""
        self.key_label.setText(f"{provider_name} API Key:")
        
        # Dynamically create the setting key name
        setting_key = f"ai_{provider_name.split(' ')[0].lower()}_api_key"
        current_key = self.settings_manager.get(setting_key, "")
        self.api_key_edit.setText(current_key)
        
        # Update info label with a link to get the key
        if "Gemini" in provider_name:
            self.info_label.setText('<a href="https://aistudio.google.com/app/apikey">Get a Google AI API key here.</a>')
        elif "OpenAI" in provider_name:
            self.info_label.setText('<a href="https://platform.openai.com/api-keys">Get an OpenAI API key here.</a>')
        elif "Groq" in provider_name:
            self.info_label.setText('<a href="https://console.groq.com/keys">Get a Groq API key here.</a>')
        elif "Anthropic" in provider_name:
            self.info_label.setText('<a href="https://console.anthropic.com/settings/keys">Get an Anthropic API key here.</a>')
        else:
             self.info_label.setText("<small>API keys are stored locally and are not shared.</small>")


    def get_settings(self) -> tuple[str, str]:
        """Returns the selected provider name and its API key."""
