...
class ThemeEditDialog(QDialog):
    """A dialog to create or edit a theme's colors."""
    def __init__(self, theme_data: dict, theme_name: str, parent=None):
        super().__init__(parent)
        self.theme_data = theme_data.copy() # Work on a copy
        self.setWindowTitle(f"Edit Theme: {theme_name}")
        self.setMinimumSize(450, 550) # Increased height
        self.setWindowIcon(get_standard_icon(QStyle.SP_DesktopIcon, "Palette")) # Find better icon

        layout = QVBoxLayout(self)
        form_layout = QFormLayout()
        self.color_editors = {}

        # User-friendly sorting of keys by their label
        sorted_keys = sorted(THEME_KEYS, key=lambda k: THEME_KEY_LABELS.get(k, k))

        for key in sorted_keys:
            label = THEME_KEY_LABELS.get(key, key)
            color_button = QPushButton(self.theme_data[key])
            color_button.setProperty("colorKey", key)
            self._update_color_button_style(color_button, QColor(self.theme_data[key]))
            color_button.clicked.connect(self._on_pick_color)
            self.color_editors[key] = color_button
            form_layout.addRow(label, color_button)
        
        # Make the form scrollable if there are many properties
    ...
    def _on_pick_color(self):
        ...
        initial_color = QColor(self.theme_data[key])

        new_color = QColorDialog.getColor(initial_color, self, f"Select Color for {THEME_KEY_LABELS.get(key, key)}")
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False, scene_ref=None, custom_snippet_manager: CustomSnippetManager | None = None):
        ...
        def add_field_with_note_and_hw_hint(form_layout, label_text, code_editor_widget, snippet_button):
            ...
            # FIX for BUG-02: Add security warning note
...
class SettingsDialog(QDialog):
    # --- FIX for BUG-02: Use constant preview sizes ---
    PREVIEW_WIDTH = 150
    PREVIEW_HEIGHT = 80
    ...
    def __init__(self, settings_manager: SettingsManager, theme_manager: ThemeManager, parent=None):
        ...
        # --- FIX for BUG-02: Use QGraphicsView for preview ---
