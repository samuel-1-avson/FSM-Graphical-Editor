...
class PySimulationUIManager(QObject):
    ...
    def __init__(self, main_window, parent=None):
        ...
        self.py_sim_continue_btn: QPushButton = None # New for breakpoints
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        py_sim_layout.setContentsMargins(4, 4, 4, 4) # Reduced margins
        py_sim_layout.setSpacing(4) # Reduced spacing

        controls_group = QGroupBox("Controls")
        controls_layout = QHBoxLayout()
        controls_layout.setSpacing(4) # Reduced spacing
        
        icon_size = QSize(18,18) # Consistent icon size
        ...
        self.py_sim_start_btn.setIconSize(icon_size)
        ...
        self.py_sim_stop_btn.setIconSize(icon_size)
        ...
        self.py_sim_reset_btn.setIconSize(icon_size)
        ...
        self.py_sim_step_btn = QPushButton("Step (Internal/Tick)")
        self.py_sim_step_btn.setIcon(get_standard_icon(QStyle.SP_MediaSeekForward, "Step"))
        self.py_sim_step_btn.setIconSize(icon_size)
        ...
        self.py_sim_continue_btn.setIconSize(icon_size)
        ...
        self.py_sim_continue_btn.setEnabled(False) 
        controls_layout.addWidget(self.py_sim_continue_btn)
        ...
        self.py_sim_event_name_edit.setObjectName("PySimEventNameEdit") # For specific styling if needed
        event_layout.addWidget(self.py_sim_event_name_edit, 1)
        
        self.py_sim_trigger_event_btn = QPushButton("Trigger") # Shortened text
        self.py_sim_trigger_event_btn.setIcon(get_standard_icon(QStyle.SP_MediaPlay, "Trg"))
        self.py_sim_trigger_event_btn.setIconSize(icon_size)
        ...
        state_group = QGroupBox("Current Status") 
        state_layout = QVBoxLayout()
        state_layout.setSpacing(2) # Reduced spacing
        self.py_sim_current_state_label = QLabel("<i>Not Running</i>"); self.py_sim_current_state_label.setStyleSheet("font-size: 9pt; padding: 2px;") # Reduced padding
        state_layout.addWidget(self.py_sim_current_state_label)

        self.py_sim_current_tick_label = QLabel("Tick: 0")
        self.py_sim_current_tick_label.setStyleSheet(f"font-size: {APP_FONT_SIZE_SMALL}; color: {COLOR_TEXT_SECONDARY}; padding: 1px 2px;") # Reduced padding
    ...
    def _update_internal_controls_enabled_state(self):
        ...
        if sim_active and self.mw.py_fsm_engine:
            is_paused_at_bp = self.mw.py_fsm_engine.paused_on_breakpoint
        # --- END NEW ---
    ...
    def update_dock_ui_contents(self):
        ...
        if self.py_sim_current_state_label:
            ...
            paused_suffix = " <b style='color:orange;'>(Paused at Breakpoint)</b>" if self.mw.py_fsm_engine.paused_on_breakpoint else ""
    ...
    def append_to_action_log(self, log_entries: list[str]):
        # ... (remains the same, already handles tick display in log entry) ...
    ...
    def on_sim_variable_changed(self, item: QTableWidgetItem):
        if not self.mw.py_fsm_engine or item.column() != 1:
            return
        ...
        simulator_instance = self.mw.py_fsm_engine
        if is_sub_var and self.mw.py_fsm_engine.active_sub_simulator:
            simulator_instance = self.mw.py_fsm_engine.active_sub_simulator
    ...
    def on_reset_py_simulation(self):
        # ... (remains the same, update_dock_ui_contents will handle tick label) ...
    ...
    def on_step_py_simulation(self):
        # ... (remains the same, logic in FSM simulator handles ticks) ...
        ...
        try:
            ...
            elif self.mw.py_fsm_engine.paused_on_breakpoint:
    ...
    def on_continue_py_simulation(self):
        ...
        if self.mw.py_fsm_engine and self.mw.py_fsm_engine.paused_on_breakpoint:
            resumed = self.mw.py_fsm_engine.continue_simulation()
            if resumed:
