...
class EditActionHandler(QObject):
    ...
    def on_auto_layout_diagram(self):
        """Triggers the auto-layout functionality for the current diagram."""
        editor = self.mw.current_editor()
        if not editor:
            return
        ...
        try:
            # This calls the method on the DiagramScene
            ...
            dialog = AutoLayoutPreviewDialog(preview_pixmap, self.mw)
            if dialog.exec():
