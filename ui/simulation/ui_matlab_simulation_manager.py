...
class MatlabSimulationUIManager(QObject):
    ...
    def _connect_signals(self):
        ...
        # --- NEW: Connect to the new live data signal from MatlabConnection ---
        if self.mw.matlab_connection:
            self.mw.matlab_connection.simulationDataUpdated.connect(self.on_live_data_update)
    ...
    @pyqtSlot(dict)
    def on_live_data_update(self, data: dict):
        """Updates the UI with live data from the stdout callback."""
        state_name = data.get('state', 'Unknown')
        sim_time = data.get('time', 0.0)

        # Re-use existing UI update methods
        if self.mw.py_sim_ui_manager:
            self.mw.py_sim_ui_manager.py_sim_current_state_label.setText(f"Live: {state_name}")
            self.mw.py_sim_ui_manager._highlight_sim_active_state(state_name)
            
        # Update progress bar if available
        if self.mw.matlab_sim_manager and self.mw.matlab_sim_manager.simulation_config:
            total_time = self.mw.matlab_sim_manager.simulation_config.stop_time
            if total_time > 0:
                progress_percent = int((sim_time / total_time) * 100)
                if hasattr(self.mw, 'progress_bar'):
                    self.mw.progress_bar.setRange(0, 100)
                    self.mw.progress_bar.setValue(progress_percent)
    ...
    def on_data_updated(self, data: SimulationData):
        """
        Updates UI elements with live data, now driven by TCP stream.
        """
        if not self.mw.matlab_sim_manager.is_simulation_active():
            return
            
        state_name = data.active_state
        
        # --- This part replaces the original on_matlab_state_update logic ---
        if self.mw.py_sim_ui_manager:
            self.mw.py_sim_ui_manager.py_sim_current_state_label.setText(f"Live: {state_name}")
            self.mw.py_sim_ui_manager._highlight_sim_active_state(state_name)

        # Update data table (optional, can be performance intensive)
        if self.data_table and self.data_table.isVisible():
            self.data_table.setRowCount(1)
            self.data_table.setItem(0, 0, QTableWidgetItem("active_state"))
            self.data_table.setItem(0, 1, QTableWidgetItem(state_name))
