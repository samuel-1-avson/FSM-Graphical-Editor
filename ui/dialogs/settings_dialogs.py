...
class SettingsDialog(QDialog):
    ...
    def __init__(self, settings_manager: SettingsManager, theme_manager: ThemeManager, parent=None):
        ...
        self.setWindowTitle(f"{config.APP_NAME} - Preferences")
        self.setWindowIcon(get_standard_icon(QStyle.StandardPixmap.SP_FileDialogDetailedView, "Prefs"))
        self.setMinimumWidth(600)
        self.setStyleSheet(f"QDialog {{ background-color: {theme_config.COLOR_BACKGROUND_DIALOG}; }} QLabel#RestartNote {{ color: {theme_config.COLOR_ACCENT_ERROR}; font-style:italic; }} QGraphicsView.previewView {{ border: 1px solid {theme_config.COLOR_BORDER_MEDIUM}; background-color: {QColor(theme_config.COLOR_BACKGROUND_LIGHT).lighter(103).name()}; }}")
        ...
        theme_restart_note.setStyleSheet(f"font-size: {config.APP_FONT_SIZE_SMALL};")
        ...
        restart_note_bh.setObjectName("RestartNote"); restart_note_bh.setStyleSheet(f"font-size: {config.APP_FONT_SIZE_SMALL};")
    ...
    def _populate_matlab_settings_tab(self, tab_widget: QWidget):
        ...
        if self.parent() and hasattr(self.parent(), 'matlab_connection'):
            ...
            if matlab_conn.matlab_path and matlab_conn.is_connected():
    ...
    def _on_theme_selection_changed(self):
        ...
        is_deletable = not self.theme_manager.is_default_theme(selected_theme)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if self.theme_manager.delete_theme(theme_name):
                ...
                if self.settings_manager.get("appearance_theme") == theme_name:
                    self.settings_manager.set("appearance_theme", "Dark")
                self._update_theme_list()
    ...
    def _update_default_item_previews(self):
        ...
        if self._preview_state_item:
            state_color_hex = self.settings_manager.get("item_default_state_color") if self.settings_manager else theme_config.COLOR_ITEM_STATE_DEFAULT_BG # Fallback

            self._preview_state_item.set_properties(
                name="State", is_initial=False, is_final=False, 
                color=state_color_hex, 
                shape_type=self.state_default_shape_combo.currentText().lower(),
                font_family=self.state_default_font_family_combo.currentFont().family(),
                font_size=self.state_default_font_size_spin.value(),
                font_bold=self.state_default_font_bold_cb.isChecked(),
                font_italic=self.state_default_font_italic_cb.isChecked(),
        ...
        if self._preview_transition_item:
            trans_color_hex = self.settings_manager.get("item_default_transition_color") if self.settings_manager else theme_config.COLOR_ITEM_TRANSITION_DEFAULT # Fallback
            
            self._preview_transition_item.set_properties(
                event="Event", 
                color=trans_color_hex,
                line_style_str=self.transition_default_line_style_combo.currentText(),
                line_width=self.transition_default_line_width_spin.value(),
                arrowhead_style=self.transition_default_arrowhead_combo.currentText().lower(),
        ...
        if self._preview_comment_item:
            self._preview_comment_item.set_properties(
                text="Comment...", 
                font_family=self.comment_default_font_family_combo.currentFont().family(),
        ...
        if dialog.exec():
            ...
            if new_color.isValid() and new_color.name() != initial_color.name(): 
                ...
                if setting_key == "item_default_state_color":
                    if self._preview_state_item:
                         self._preview_state_item.set_properties(name="State", is_initial=False, is_final=False, color=new_color.name())
                elif setting_key == "item_default_transition_color":
                    if self._preview_transition_item:
                        self._preview_transition_item.set_properties(event="Event", color=new_color.name())
                self.update()
    ...
    def _matlab_auto_detect(self):
        if self.parent() and hasattr(self.parent(), 'matlab_connection'):
            ...
            self.matlab_test_status_label.setStyleSheet(f"font-style: italic; color: {theme_config.COLOR_TEXT_SECONDARY}; background-color: {QColor(theme_config.COLOR_ACCENT_PRIMARY_LIGHT).lighter(120).name()};")
    ...
    def _matlab_update_test_label(self, success, message):
        ...
        if success:
            current_style += f"color: {theme_config.COLOR_ACCENT_SUCCESS}; background-color: {QColor(theme_config.COLOR_ACCENT_SUCCESS).lighter(180).name()};"
        else:
            current_style += f"color: {theme_config.COLOR_ACCENT_ERROR}; background-color: {QColor(theme_config.COLOR_ACCENT_ERROR).lighter(180).name()};"
