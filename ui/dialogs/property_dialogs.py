...
class AiHelperMixin:
    ...
    def _on_ai_helper_clicked(self, target_widget, code_type):
        """Handles the AI helper button click, showing a dialog and sending a prompt."""
...
class StatePropertiesDialog(QDialog, AiHelperMixin):
    def __init__(self, settings_manager, asset_manager, parent=None, current_properties=None, is_new_state=False, scene_ref=None):
        ...
        self.setStyleSheet(f"QDialog {{ background-color: {theme_config.COLOR_BACKGROUND_DIALOG}; }} QLabel#SafetyNote, QLabel#HardwareHintLabel {{ font-size: {config.APP_FONT_SIZE_SMALL}; color: {theme_config.COLOR_TEXT_SECONDARY}; }} QGroupBox {{ background-color: {QColor(theme_config.COLOR_BACKGROUND_LIGHT).lighter(102).name()}; }}")
        ...
        self.current_color = QColor(p.get('color', theme_config.COLOR_ITEM_STATE_DEFAULT_BG)); self._update_color_button_style()
        self.color_button.clicked.connect(self._choose_color)
        appearance_layout.addRow("Display Color:", self.color_button)

        self.shape_combo = QComboBox()
        self.shape_combo.addItems(["Rectangle", "Ellipse"])
        self.shape_combo.setCurrentText(p.get('shape_type', config.DEFAULT_STATE_SHAPE).capitalize())
        appearance_layout.addRow("Shape:", self.shape_combo)

        font_group = QGroupBox("Label Font")
        font_layout = QFormLayout(font_group)
        self.font_family_combo = QFontComboBox()
        self.font_family_combo.setCurrentFont(QFont(p.get('font_family', config.APP_FONT_FAMILY)))
        ...
        default_border_style_str = p.get('border_style_str', SettingsManager.QT_PEN_STYLE_TO_STRING.get(config.DEFAULT_STATE_BORDER_STYLE, "Solid"))
        self.border_style_combo.setCurrentText(default_border_style_str)
        border_layout.addRow("Style:", self.border_style_combo)
        self.border_width_spin = QDoubleSpinBox(); self.border_width_spin.setRange(0.5, 10.0); self.border_width_spin.setSingleStep(0.1); self.border_width_spin.setValue(p.get('border_width', config.DEFAULT_STATE_BORDER_WIDTH)); self.border_width_spin.setDecimals(1)
        ...
        self.icon_preview_label.setStyleSheet(f"border: 1px solid {theme_config.COLOR_BORDER_MEDIUM}; background-color: {QColor(theme_config.COLOR_BACKGROUND_LIGHT).lighter(105).name()};")
        ...
        self.action_language_combo.setCurrentText(p.get('action_language', config.DEFAULT_EXECUTION_ENV))
        ...
        def add_field_with_note_and_hw_hint(form_layout, label_text, code_editor_widget, snippet_button, ai_button):
            ...
            hw_hint_label.setObjectName("HardwareHintLabel"); hw_hint_label.setStyleSheet(f"color: {theme_config.COLOR_TEXT_SECONDARY}; font-style: italic; font-size: 7.5pt;")
        ...
        hint_label.setStyleSheet(f"color: {theme_config.COLOR_TEXT_SECONDARY}; font-size: {config.APP_FONT_SIZE_SMALL};")
    ...
    def _update_snippet_button_menu(self, button: QPushButton, target_widget: CodeEditor, language_mode: str, snippet_category: str):
        ...
        if self.asset_manager: 
            custom_snippets_dict = self.asset_manager.get_custom_snippets(language_mode, snippet_category)
...
class TransitionPropertiesDialog(QDialog, AiHelperMixin):
    def __init__(self, asset_manager: AssetManager, parent=None, current_properties=None, is_new_transition=False):
        super().__init__(parent)
        self.setWindowTitle("Transition Properties")
        self.setWindowIcon(get_standard_icon(QStyle.StandardPixmap.SP_FileDialogInfoView, "Props"))
        ...
        self.action_ai_btn = self._create_ai_helper_button(self.action_edit, "action")
        ...
        self.color_button = QPushButton("Choose Color..."); self.color_button.setObjectName("ColorButton")
        self.current_color = QColor(p.get('color', theme_config.COLOR_ITEM_TRANSITION_DEFAULT)); self._update_color_button_style()
        self.color_button.clicked.connect(self._choose_color)
        appearance_layout.addRow("Display Color:", self.color_button)
        ...
        self.line_style_combo.setCurrentText(p.get('line_style_str', SettingsManager.QT_PEN_STYLE_TO_STRING.get(config.DEFAULT_TRANSITION_LINE_STYLE, "Solid")))
        ...
        self.line_width_spin.setValue(p.get('line_width', config.DEFAULT_TRANSITION_LINE_WIDTH)); self.line_width_spin.setDecimals(1)
        ...
        self.arrowhead_style_combo.setCurrentText(p.get('arrowhead_style', config.DEFAULT_TRANSITION_ARROWHEAD).capitalize())
        ...
        self.label_font_family_combo.setCurrentFont(QFont(p.get('label_font_family', config.APP_FONT_FAMILY)))
    ...
    def _create_insert_snippet_button_lineedit(self, target_line_edit: QLineEdit, snippet_category: str, button_text="Insert..."):
        button = QPushButton(button_text); button.setObjectName("SnippetButton")
        button.setIcon(get_standard_icon(QStyle.StandardPixmap.SP_FileDialogContentsView,"InsSnip")); button.setIconSize(QSize(16,16))
...
class CommentPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None):
        ...
        self.setWindowIcon(get_standard_icon(QStyle.StandardPixmap.SP_MessageBoxInformation, "Cmt"))
        self.setStyleSheet(f"QDialog {{ background-color: {theme_config.COLOR_BACKGROUND_DIALOG}; }}")
        ...
        current_font = QFont(p.get('font_family', config.APP_FONT_FAMILY), p.get('font_size', 9))
...
class SubFSMEditorDialog(QDialog):
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        self.setWindowIcon(get_standard_icon(QStyle.StandardPixmap.SP_FileDialogDetailedView, "SubEdit"))
        self.setMinimumSize(800, 600)
        self.setStyleSheet(f"QDialog {{ background-color: {theme_config.COLOR_BACKGROUND_DIALOG}; }} QLabel#ErrorLabel {{ color: {theme_config.COLOR_ACCENT_ERROR}; font-weight: bold; }}")
        ...
        if IMPORTS_SUCCESSFUL: 
            ...
            actions_data = [
                ("select", "Select/Move", QStyle.StandardPixmap.SP_ArrowRight, "SelSub"),
                ("state", "Add State", QStyle.StandardPixmap.SP_FileDialogNewFolder, "StSub"),
                ("transition", "Add Transition", QStyle.StandardPixmap.SP_ArrowForward, "TrSub"),
                ("comment", "Add Comment", QStyle.StandardPixmap.SP_MessageBoxInformation, "CmSub")
            ]
            ...
            self.sub_undo_action.setIcon(get_standard_icon(QStyle.StandardPixmap.SP_ArrowBack, "UnSub"))
            ...
            self.sub_redo_action.setIcon(get_standard_icon(QStyle.StandardPixmap.SP_ArrowForward, "ReSub"))
            ...
            self.status_label.setStyleSheet(f"font-size: {config.APP_FONT_SIZE_SMALL}; color: {theme_config.COLOR_TEXT_SECONDARY};")
        else: 
            ...
            error_detail_label.setStyleSheet(f"color: {theme_config.COLOR_TEXT_SECONDARY}; margin-bottom: 5px;")
            ...
            self.json_text_edit.setLineWrapMode(CodeEditor.LineWrapMode.NoWrap) 
            try:
               ...
               isinstance(updated_data.get('comments'), list):
                ...
                if sub_states_list:
                    ...
                    if not has_initial:
                        ...
                        save_anyway_btn = msg_box.addButton("Save Anyway", QMessageBox.ButtonRole.AcceptRole)
                        set_first_btn = msg_box.addButton("Set First as Initial & Save", QMessageBox.ButtonRole.YesRole)
