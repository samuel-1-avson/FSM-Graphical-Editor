...
public:
    ...
    void step(const std::string &event_name)
    {
        action_log_.clear();
        current_tick_++;

        State *current_leaf_state = current_state_path_.back();
        executeAction(current_leaf_state->during_action);

        for (const auto &trans : transitions_)
        {
            if (trans.source == current_leaf_state->name && trans.event == event_name)
            {
                // In C++, condition evaluation is delegated to Python wrapper
                // For now, we assume true if condition exists, or if no condition.
                // The Python wrapper will perform the actual check.
                logAction("EVAL_CONDITION", trans.condition);

                // Assuming condition is met (Python will verify)
                logAction("EXIT_STATE", current_leaf_state->exit_action);
                logAction("TRANSITION_ACTION", trans.action);
...
private:
    void enterState(State *state)
    {
        ...
        logAction("ENTRY_STATE", state->entry_action);
    }
...
{
    delete static_cast<FsmSimulator *>(handle);
}
...
{
    static_cast<FsmSimulator *>(handle)->reset();
}

FSM_API void step(FSM_HANDLE handle, const char *event_name)
{
    static_cast<FsmSimulator *>(handle)->step(event_name ? std::string(event_name) : "");
}
...
{
    return static_cast<FsmSimulator *>(handle)->getCurrentTick();
}
